...
class LocationService {
  ...
  public async onAppStopped(userId: string, packageName: string): Promise<void> {
    ...
    if (NAVIGATION_APP_PACKAGES.includes(packageName)) {
      ...
      if (user) {
        // Create a new list of running apps with the stopped app removed
        // to avoid the race condition with the database update.
        const stillRunningApps = user.runningApps.filter(app => app !== packageName);
        
        // Check if any *other* navigation app is still in the list.
        ...
        if (!anotherNavAppIsRunning) {
         ...
         // Fallback in case user is not found, revert to standard to be safe.
