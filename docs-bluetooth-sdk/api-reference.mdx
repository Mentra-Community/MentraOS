...
| `startOtaUpdate(otaVersionUrl?)` | `OtaStartAckEvent` from `ota_start_ack`. Omitting `otaVersionUrl` uses the glasses production default manifest. | Start-ack timeout, send failure, invalid manifest URL, or another OTA start in flight. |
...
| `startOtaUpdate(otaVersionUrl?)` | `ota_start` | Start OTA after your app presents the update and the user accepts it. Optional HTTP(S) manifest URL; omit it to use the glasses production default. Resolves with `OtaStartAckEvent` from the ASG client. |
