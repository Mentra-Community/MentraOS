...
| Gallery/button settings | `SettingsAckEvent` with a non-failure status such as `applied` or `ready`. | `settings_ack.status` of `error`, `failed`, `failure`, or `rejected`; timeout; or send failure. |
| `setCameraFov(...)` | `CameraFovResult` with `ready: true`, `hardwareApplied: true`, `fov`, `roiPosition`, and `requestId` after the restarted camera is ready. | Glasses-side FOV/settings error, missing readiness after camera restart, timeout, or send failure. |
...
| `requestPhoto(...)` | ASG acceptance response: React Native `PhotoSuccessResponseEvent`; Android/iOS successful `PhotoResponseEvent`. Camera-busy, low-battery, storage, and FOV-restart rejections are decided before this success response; capture/upload progress follows through `photo_status`. | Raw `photo_response.state === "error"`, acceptance timeout, or send failure. |
| `startVideoRecording(...)` | React Native `VideoRecordingSuccessStatusEvent` with `status: "recording_started"`; Android/iOS successful `VideoRecordingStatusEvent`. | `success: false` statuses such as `already_recording`, timeout, send failure, or duplicate request ID. |
| `stopVideoRecording(requestId)` | React Native `VideoRecordingSuccessStatusEvent` with `status: "recording_stopped"`; Android/iOS successful `VideoRecordingStatusEvent`. | `success: false` statuses such as `not_recording`, timeout, send failure, or duplicate request ID. |
...
| `stopStream()` | `StreamStatusEvent` once `status` is `stopped`. | `stream_status` error, timeout, send failure, or another stream stop in flight. |
...
| `photo_response` | `PhotoResponseEvent` | Photo request is accepted or rejected by the glasses. `requestPhoto(...)` resolves with the success case and rejects on the error case; capture/upload progress follows through `photo_status`. |
...
| `video_recording_status` | `VideoRecordingStatusEvent` | Video recording start/stop succeeds or fails. `startVideoRecording(...)` and `stopVideoRecording(...)` resolve with `success: true`; unsuccessful statuses such as `already_recording` reject the promise. |
...
For request/response commands, prefer the returned value from the method, such as the `PhotoSuccessResponseEvent` returned by React Native `requestPhoto(...)` or the `VideoRecordingSuccessStatusEvent` returned by React Native `startVideoRecording(...)`. `requestPhoto(...)` only confirms that the glasses accepted the request and know where to upload; camera-busy and other command rejections happen before that success response, and capture/upload/Bluetooth fallback progress follows through `photo_status`. Use event listeners for ongoing hardware streams and status updates: for example, `useBluetoothEvent('mic_pcm', ...)` receives a `MicPcmEvent`. `MicPcmEvent` includes `sampleRate`, `bitsPerSample`, `channels`, `encoding`, and `voiceActivityDetectionEnabled`; `MicLc3Event` includes `sampleRate`, `channels`, `encoding`, `frameDurationMs`, `frameSizeBytes`, `bitrate`, `packetizedFromGlasses`, and `voiceActivityDetectionEnabled`. `speaking_status` is separate from microphone audio frames so apps can use continuous audio while still reacting to speech activity.

Public React Native event payload fields usually use camelCase. OTA events intentionally mirror the glasses firmware field names, such as `overall_percent` and `version_name`. For example, touch events expose `deviceModel` and `gestureName`, successful photo responses expose `uploadUrl`, hotspot errors expose `errorMessage`, Wi-Fi scan events expose `scanComplete`, and gallery status exposes `hasContent`, `cameraBusy`, and optional `cameraBusyReason`.
