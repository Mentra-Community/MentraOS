...
The SDK can report anonymous, opt-in usage events to Mentra's PostHog project:

| Event | Fires When |
| --- | --- |
| `bluetooth_sdk_started` | Once per app runtime after analytics is configured for that SDK instance. |
...
Analytics is disabled by default. It starts only when the consuming app passes a
PostHog project API key. The key is PostHog's public project write token, not a
private PostHog personal API key.
...
val sdk = MentraBluetoothSdk.create(
    context = applicationContext,
    config = MentraBluetoothSdkConfig(
        analytics = BluetoothSdkAnalyticsConfig(
            postHogApiKey = BuildConfig.MENTRA_BLUETOOTH_POSTHOG_API_KEY,
            postHogHost = BuildConfig.MENTRA_BLUETOOTH_POSTHOG_HOST,
        )
    ),
    listener = listener,
)
...
let sdk = MentraBluetoothSDK(
    configuration: MentraBluetoothSDKConfiguration(
        analytics: BluetoothSdkAnalyticsConfiguration(
            postHogApiKey: Bundle.main.object(forInfoDictionaryKey: "MENTRA_BLUETOOTH_POSTHOG_API_KEY") as? String,
            postHogHost: Bundle.main.object(forInfoDictionaryKey: "MENTRA_BLUETOOTH_POSTHOG_HOST") as? String
...
Use `https://us.i.posthog.com` for US Cloud, `https://eu.i.posthog.com` for EU
Cloud, or the ingestion host for a self-hosted PostHog instance.
