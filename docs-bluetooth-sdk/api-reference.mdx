...
| `video_recording_status` | `VideoRecordingStatusEvent` | Video recording start/stop succeeds or fails. Also returned by `startVideoRecording(...)` and `stopVideoRecording(...)`; unsuccessful statuses such as `already_recording` reject the promise. |
