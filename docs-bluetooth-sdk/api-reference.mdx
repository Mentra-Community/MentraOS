...
| Gallery/button settings | `SettingsAckEvent` with a non-failure status such as `applied` or `ready`; the SDK local settings store updates only after this ASG ack resolves. | `settings_ack.status` of `error`, `failed`, `failure`, or `rejected`; timeout; or send failure. |
| `setCameraFov(...)` | `CameraFovResult` with `ready: true`, `hardwareApplied: true`, `fov`, `roiPosition`, and `requestId` after the restarted camera is ready; the SDK local FOV setting updates only after this result resolves. | Glasses-side FOV/settings error, missing readiness after camera restart, timeout, or send failure. |
...
| `requestPhoto(...)` | Terminal response after capture and delivery finish: React Native `PhotoSuccessResponseEvent`; Android/iOS successful `PhotoResponseEvent`. The success includes `uploadUrl`, and may include webhook-returned `photoUrl`, `statusUrl`, `bytes`, `size`, `mimeType`, or `contentType`. Progress follows through `photo_status`. | Raw `photo_response.state === "error"` from glasses/phone, fallback upload failure, terminal response timeout, or send failure. |
