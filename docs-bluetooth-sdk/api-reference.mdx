...
| Gallery/button settings | `SettingsAckEvent` with a non-failure status such as `applied` or `ready`. | `settings_ack.status` of `error`, `failed`, `failure`, or `rejected`; timeout; send failure; or duplicate request ID. |
| `setCameraFov(...)` | `CameraFovResult` with `ready: true`, `hardwareApplied: true`, `fov`, `roiPosition`, and `requestId` after the restarted camera is ready. | Glasses-side FOV/settings error, missing readiness after camera restart, timeout, send failure, or duplicate request ID. |
...
| `requestPhoto(...)` | React Native `PhotoSuccessResponseEvent`; Android/iOS successful `PhotoResponseEvent`. | Raw `photo_response.state === "error"`, timeout, send failure, or duplicate request ID. |
...
| `startStream(...)` | `StreamStatusEvent` for the requested stream once `status` is `streaming`. | `stream_status` error/reconnect failure, timeout, send failure, or duplicate stream start. |
...
| `rgbLedControl(...)` | React Native `RgbLedControlSuccessResponseEvent`; Android/iOS successful `RgbLedControlResponseEvent`. | Raw `rgb_led_control_response.state === "error"`, timeout, send failure, or duplicate request ID. |
...
| `photo_response` | `PhotoResponseEvent` | Photo request succeeds or fails. `requestPhoto(...)` resolves with the success case and rejects on the error case. |
...
For request/response commands, prefer the returned value from the method, such as the `PhotoSuccessResponseEvent` returned by React Native `requestPhoto(...)` or the `VideoRecordingSuccessStatusEvent` returned by React Native `startVideoRecording(...)`. Use event listeners for ongoing hardware streams and status updates: for example, `useBluetoothEvent('mic_pcm', ...)` receives a `MicPcmEvent`. `MicPcmEvent` includes `sampleRate`, `bitsPerSample`, `channels`, `encoding`, and `voiceActivityDetectionEnabled`; `MicLc3Event` includes `sampleRate`, `channels`, `encoding`, `frameDurationMs`, `frameSizeBytes`, `bitrate`, `packetizedFromGlasses`, and `voiceActivityDetectionEnabled`. `speaking_status` is separate from microphone audio frames so apps can use continuous audio while still reacting to speech activity.
