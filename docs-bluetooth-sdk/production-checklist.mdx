...
- Decide whether your app should send Mentra Bluetooth SDK usage analytics.
- If enabled, configure `MENTRA_BLUETOOTH_POSTHOG_API_KEY` with the PostHog project API key and `MENTRA_BLUETOOTH_POSTHOG_HOST` with the ingestion host (`https://us.i.posthog.com` or `https://eu.i.posthog.com` for PostHog Cloud).
- Verify `bluetooth_sdk_started` appears after app startup and `bluetooth_sdk_glasses_connected` appears only after a successful glasses connection.
- Confirm your privacy copy reflects anonymous SDK usage analytics if your app enables it.
