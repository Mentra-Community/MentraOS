...
The Android artifact is published on Maven Central. Keep `google()` and JitPack in `repositories` for transitive runtime dependency resolution; the SDK's public Android API does not expose Google Play Services or JitPack-only types. The companion LC3 codec artifact resolves transitively as `com.mentraglass:lc3Lib`.
...
<CardGroup cols={2}>
  <Card title="Run Example Apps" icon="mobile" href="/examples">
