...
class MentraBluetoothSdk private constructor(
    ...
    companion object {
        ...
        // stopAllServices() 闁?the point where every older start is actually
...
private fun requireBluetoothReady(operation: String) {
    ...
    private fun handleStreamStatusForRequests(event: StreamStatusEvent) {
        ...
        if (startMatch != null) {
            ...
            // BEFORE stopAllServices, so older starts were not preempted 闁?and
            ...
            when (event.state) {
                ...
                StreamState.ERROR -> {
                    if (!event.streamId.isNullOrBlank() && event.willRetry == true) {
                        ...
                        // start pending for the retry's verdict 闁?`reconnected` or
                        ...
                        // fail fast here 闁?the shipped pre-#3487 Android behavior.
                        pendingStreamStarts.remove(streamId, start)
    ...
    // the id-less heuristic deliberately ignores 闁?without this they would run
    ...
    private fun matchingStreamStart(event: StreamStatusEvent): Pair<String, PendingStreamStart>? {
        ...
        if (event.state == StreamState.ERROR) {
            ...
            // statuses arrive async 闁?so with overlapping starts the wire is
        ...
        if (pendingStreamStarts.size == 1) {
            ...
            // pending start 闁?a start_stream that replaces a running publisher
    ...
    private fun handleWifiStatusForRequests(event: WifiStatusEvent) {
        ...
        // request timeout. Only the error field counts as failure 闁?the glasses'
