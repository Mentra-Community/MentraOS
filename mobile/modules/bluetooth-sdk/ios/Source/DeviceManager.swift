...
@objc(DeviceManager) class DeviceManager: NSObject {
    ...
    func requestPhoto(
        ...
        Bridge.log(
            "MAN: PHOTO PIPELINE [4/6] DeviceManager.requestPhoto requestId=\(requestId) appId=\(appId) webhookUrl=\(webhookUrl ?? "nil") size=\(size) compress=\(compress ?? "none") flash=\(flash) save=\(save) sound=\(sound) exposureTimeNs=\(exposureTimeNs.map { String($0) } ?? "nil") iso=\(manualIso.map { String($0) } ?? "auto") sgc=\(sgc != nil ? String(describing: type(of: sgc!)) : "null")"
        )
        guard let sgc else {
            Bridge.log(
        ...
        sgc.requestPhoto(
            ...
            compress: compress, flash: flash, save: save, sound: sound, exposureTimeNs: exposureTimeNs, iso: manualIso
        )
    }
