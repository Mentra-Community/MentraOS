...
private enum EvenHubProto {
    ...
    /// Build a ListItemContainerProperty message
    static func listItemContainerProperty(
        itemCount: Int32, itemWidth: Int32 = 0, isItemSelectBorderEn: Bool = true,
        itemName: [String]
    ) -> Data {
        var w = ProtobufWriter()
        w.writeInt32Field(1, itemCount)
        w.writeInt32Field(2, itemWidth)
        w.writeInt32Field(3, isItemSelectBorderEn ? 1 : 0)
        for item in itemName {
            w.writeStringField(4, item)
        }
        return w.data
    }

    /// Build a ListContainerProperty message
    static func listContainerProperty(
        x: Int32, y: Int32, width: Int32, height: Int32,
        borderWidth: Int32 = 1, borderColor: Int32 = 13, borderRadius: Int32 = 6,
        paddingLength: Int32 = 5, containerID: Int32, containerName: String? = nil,
        itemContainer: Data, isEventCapture: Bool = false
    ) -> Data {
        var w = ProtobufWriter()
...
class G2: NSObject, SGCManager {
    ...
    private struct ListContainer: Equatable {
        ...
        func matches(
            x: Int32, y: Int32, width: Int32, height: Int32, borderWidth: Int32,
            borderColor: Int32, borderRadius: Int32, paddingLength: Int32, itemWidth: Int32,
            showSelectionBorder: Bool, items: [String]
        ) -> Bool {
            self.x == x && self.y == y && self.width == width && self.height == height
                && self.borderWidth == borderWidth && self.borderColor == borderColor
                && self.borderRadius == borderRadius && self.paddingLength == paddingLength
                && self.itemWidth == itemWidth && self.showSelectionBorder == showSelectionBorder
                && self.items == items
        }
    }
    ...
    private let listContainerIDPool: [Int32] = [7, 8]
    ...
    func sendSelectableList(
        ...
        if items.isEmpty {
            ...
            normalizedItems = items.prefix(20).map { item in
                let clipped = String(item.prefix(64))
                return clipped.isEmpty ? " " : clipped
            }
        }

        if listContainers.firstIndex(where: {
            $0.matches(
                x: x, y: y, width: width, height: height, borderWidth: borderWidth,
                borderColor: borderColor, borderRadius: borderRadius,
                paddingLength: paddingLength, itemWidth: itemWidth,
                showSelectionBorder: showSelectionBorder, items: normalizedItems
            )
        }) != nil, textContainers.isEmpty, imageContainers.isEmpty, pageCreated {
            return
        }
        ...
        let id = listContainerIDPool[0]
        listContainers.append(
            ListContainer(
                id: id, x: x, y: y, width: width, height: height, borderWidth: borderWidth,
                borderColor: borderColor, borderRadius: borderRadius,
                paddingLength: paddingLength, itemWidth: itemWidth,
                showSelectionBorder: showSelectionBorder, items: normalizedItems
            )
    ...
    func sendTextAt(
        ...
        let hadListContainers = !listContainers.isEmpty
        ...
        if let i = textContainers.firstIndex(where: {
            ...
            if hadListContainers {
                Bridge.log(
                    "G2: sendText() - replacing list page with text container \(container.id)"
                )
                await rebuildPage()
                return
            }
    ...
    private func createPageWithContainers() {
        var listContainerProps: [Data] = []
        for (index, c) in listContainers.enumerated() {
            let itemContainer = EvenHubProto.listItemContainerProperty(
                itemCount: Int32(c.items.count),
                itemWidth: c.itemWidth,
                isItemSelectBorderEn: c.showSelectionBorder,
                itemName: c.items
            )
            listContainerProps.append(
                EvenHubProto.listContainerProperty(
                    x: c.x, y: c.y, width: c.width, height: c.height,
                    borderWidth: c.borderWidth, borderColor: c.borderColor,
                    borderRadius: c.borderRadius, paddingLength: c.paddingLength,
                    containerID: c.id, containerName: c.name, itemContainer: itemContainer,
        ...
        for (index, c) in textContainers.enumerated() {
            textContainerProps.append(
                EvenHubProto.textContainerProperty(
                    ...
                    containerName: c.name, isEventCapture: listContainers.isEmpty && index == 0,
                    content: c.content
                )
            )
        }
    ...
    private func handleTouchEvent(_ devEventData: Data) {
        ...
        if let listData = fields[1] as? Data {
            ...
            if let eventTypeRaw = listFields[5] as? Int32,
               let parsedEventType = OsEventType(rawValue: eventTypeRaw)
            {
