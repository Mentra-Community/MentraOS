...
lowercase `[a-z0-9_-]`, at most 32 characters, and reported as `app_environment`.
...
`simulator`, `sideload`, a named third-party store, or `other_store`), the raw
Android `app_installer_package` when present, and `app_environment` when the
host declares one. Connection and identification events include `fully_booted`,
...
`glasses_android_version`, `glasses_app_version`, `glasses_build_number`) so a
missing or malformed serial can be traced to the firmware that produced it. This serial identifies the glasses
