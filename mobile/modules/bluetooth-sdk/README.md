...
const savedNetworks = await BluetoothSdk.getSavedWifiNetworks()
...
`requestWifiScan()` resolves from the ASG `wifi_scan_result` completion response with the updated scan list, including `[]` when no networks are found. Intermediate `wifi_scan_result` events can arrive with `scanComplete: false` while the glasses stream discovered networks; the final event uses `scanComplete: true`. If older glasses stream non-empty scan results but never send the completion event, the request resolves with the accumulated scan list when the request times out. `sendWifiCredentials()` resolves when the requested SSID is connected, `forgetWifiNetwork()` resolves when ASG dispatches the forget command (and returns the current link snapshot), `setHotspotState()` resolves when the requested hotspot state is reported, and `requestVersionInfo()` resolves from the ASG `version_info` response instead of local store changes.

Current Mentra Live builds return an exact-ID `wifi_forget_result`, so `dispatched: true` resolves promptly without occupying the WiFi command slot for the generic 15-second timeout. On K900 this confirms only that ASG queued the asynchronous vendor broadcast; it does not verify credential removal. The SDK gives that correlated result 750 ms of priority over a matching legacy `wifi_status`, then resolves from the legacy status so older glasses remain responsive. `getSavedWifiNetworks()` preserves exact SSID identity and returns configured SSIDs on managers with a reliable enumeration API. K900 rejects with `list_saved_networks_unsupported` because its vendor credential store has no result callback; older glasses that ignore `request_saved_wifi_networks` reject with the normal request timeout.
