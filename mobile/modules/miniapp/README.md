...
| `session.camera`              | `takePhoto({size?, mode?, sound?, saveToGallery?, saveToCameraRoll?, zsl?, mfnr?, timeoutMs?, ...})`, `setFov({fov, roiPosition?} \| {preset})`, `hasPermission`                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
`await session.camera.takePhoto(...)` resolves only after the glasses have transferred the photo to the phone over Bluetooth. The result includes `{requestId, dataUrl, photoUrl, mimeType, size}` where `dataUrl` is the JPEG inline (`data:image/jpeg;base64,...`) and `photoUrl` carries the same value for rendering (the background runtime's `fetch` does not accept data URLs — decode `dataUrl` for bytes); no cloud or network request is in the path. Glasses-side or transfer failures such as `CAMERA_BUSY`, `BATTERY_LOW`, or storage errors reject the promise; intermediate `photo_status` progress and request acceptance alone do not resolve it.

`saveToGallery` keeps a copy in the glasses gallery and `saveToCameraRoll` also exports the photo to the phone's OS camera roll. `transferMethod` and `compress` are accepted for compatibility but ignored: delivery is always Bluetooth to the phone, and the Bluetooth transport codec governs quality.
