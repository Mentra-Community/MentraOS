...
CI releases are owned by `.github/workflows/coordinated-release.yml`. These
scripts remain useful for supervised local builds and diagnostics.

| Script                  | Purpose                                                                                                                                                                   |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `release-all.mjs`       | Convenience wrapper: runs `release-android.mjs` then `release-ios.mjs`. `bun run release:all`.                                                                            |
| `release-android.mjs`   | Legacy supervised Android release helper. Coordinated CI uses the reusable mobile workflow.                                                                               |
| `release-ios.mjs`       | Legacy supervised iOS release helper. Coordinated CI uses the reusable mobile workflow.                                                                                   |
...
4. **Confirm `ccache` is installed** (`command -v ccache`). The iOS PR build (`.github/workflows/mentra-app-ios-build.yml`) enables React Native's ccache wrappers on pull_request builds when the binary is present. The cache lives in `~/.ccache-mentra-ci/<namespace>/`, where the namespace is a hash of Xcode, macOS, arch, ccache version and the repo's ccache config, so a toolchain update starts a fresh namespace; each namespace is capped at 20 GB by `.github/ccache/ios-compile-check.conf`. All runner processes on the host share it. Without ccache the workflow still passes, just slower. On an existing runner that predates this: `brew install ccache`.
...
```bash
gh workflow run coordinated-release.yml --ref dev -f dry_run=true
```
...
A Linux runner supports Android jobs but not iOS. The coordinated release keeps
the ASG and iOS jobs on Apple Silicon runners; move only explicitly portable
jobs after validating the complete release contract on the new runner.
