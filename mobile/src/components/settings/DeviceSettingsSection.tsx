 ...
 * paired device's name 闁?matching Parth's flattened Settings design. Every row
 * is conditional on the device's capabilities; for a connected Even Realities
 * G1 the visible order is display position 闁?dashboard 闁?auto-brightness 闁?
 * microphone 闁?device info 闁?disconnect 闁?unpair (the design's reference
...
export function DeviceSettingsSection() {
  ...
  // Button-action settings are no longer surfaced in the UI 闁?the action button always launches the
  ...
  // No glasses paired at all 闁?no device section to show.
  if (!defaultWearable) {
    return null
  }
  ...
  return (
    <View style={{gap: theme.spacing.s2}}>
      ...
      {/* Dashboard 闁?glasses with a MentraOS-managed display dashboard only.
      ...
      {/* Glasses Menu 闁?G2 only, requires connection */}
      ...
      {/* Layout settings 闁?super mode (display layout tooling) */}
      {superMode && (
        <RouteButton label={translate("settings:layoutSettings")} onPress={() => push("/miniapps/settings/layout")} />
      )}

      {/* Button Settings 闁?Mentra Live only (G2's button is a touchpad and conflicts with the native menu).
      ...
      {/* OTA Progress 閳?OTA-capable glasses in super mode */}
      ...
      {/* Nex Developer Settings 闁?Mentra Display only */}
