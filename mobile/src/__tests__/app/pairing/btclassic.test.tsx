...
describe("btclassic pairing screen", () => {
  ...
  it("kicks off the connect and reveals loading once Bluetooth Classic links", async () => {
    ...
    await waitFor(() => {
      expect(toolkit.glasses.connect).toHaveBeenCalledWith(device, {saveAsDefault: false})
  ...
  it("routes a connect kickoff rejection to the failure screen", async () => {
    ;(toolkit.glasses.connect as jest.Mock).mockRejectedValueOnce(new Error("bluetooth powered off"))
