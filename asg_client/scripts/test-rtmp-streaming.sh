#!/bin/bash
# test-rtmp-streaming.sh — Manual test driver for RtmpStreamingService (RTMP)
#
# Usage:
#   ./test-rtmp-streaming.sh start <rtmp_url> [stream_id]
#   ./test-rtmp-streaming.sh stop
#   ./test-rtmp-streaming.sh logs            # tail logcat (Ctrl-C to exit)
#   ./test-rtmp-streaming.sh status          # check if service process is running
#
# Prerequisites:
...
SERVICE_CLASS="com.mentra.asg_client.io.streaming.services.RtmpStreamingService"
LOG_TAG="RtmpStreamingService"
...
cmd_start() {
  ...
  if [ -z "$rtmp_url" ]; then
    echo "Usage: $0 start <rtmp_url> [stream_id]" >&2
    exit 1
  fi
  ...
  echo "→ Starting RtmpStreamingService…"
  adb shell am start-foreground-service \
    -n "${PKG}/${SERVICE_CLASS}" \
    --es rtmp_url  "$rtmp_url" \
...
cmd_stop() {
  ...
  echo "→ Stopping RtmpStreamingService (${PKG})…"
...
cmd_logs() {
  ...
  adb logcat -v time \
    "$LOG_TAG:D" \
    "RtmpStreamingService:D" \
    "StreamCommandHandler:D" \
    "*:S"
}
...
cmd_status() {
  ...
  adb shell dumpsys activity services "$PKG" 2>/dev/null \
    | grep -E "(ServiceRecord|running|foreground|Rtmp)" || echo "(none found)"
...
case "${1:-}" in
  ...
  status) cmd_status ;;
  *)
    echo "RTMP streaming test script"
    echo ""
    echo "Usage:"
    echo "  $0 start <rtmp_url> [stream_id]   Start streaming to an RTMP endpoint"
