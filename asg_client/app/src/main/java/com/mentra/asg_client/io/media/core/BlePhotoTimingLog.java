...
public final class BlePhotoTimingLog {
    ...
    static String formatPhaseBreakdown(
        ...
        appendCropTimeTradeoffSection(sb, payloadBytes, reduction, uart, cropMs, compressMs);
    ...
    /**
     * Compares crop-phase wall time against TX time saved versus a no-crop BLE encode (same
     * codec/quality/downscale target), using measured UART speed. Do not compare against the raw
     * sensor JPEG — that path never goes over BLE unprocessed.
     */
    private static void appendCropTimeTradeoffSection(
            StringBuilder sb,
            long payloadBytes,
            @Nullable PayloadReductionStats reduction,
            @Nullable UartTransferStats uart,
            long cropPhaseMsFromTimings,
            long compressMs) {
        if (reduction == null || !reduction.cropAttempted) {
            return;
        }
        long cropMs =
                reduction.cropDurationMs > 0 ? reduction.cropDurationMs : cropPhaseMsFromTimings;
        boolean baselineMeasured = reduction.measuredFullFrameBytes() > 0;
        long baselineBytes = baselineFullFrameBlePayloadBytes(payloadBytes, reduction);
        long cropSavedBytes =
                reduction.cropApplied && baselineBytes > payloadBytes
                        ? baselineBytes - payloadBytes
                        : 0L;
        double uartBytesPerMs = 0.0;
        if (uart != null && uart.durationMs > 0 && uart.payloadBytes > 0) {
            uartBytesPerMs = (double) uart.payloadBytes / (double) uart.durationMs;
        }

        String baselineTag = baselineMeasured ? "measured" : "est.";

        sb.append('\n');
        sb.append("  CROP TIME TRADEOFF");
        sb.append(baselineMeasured
                ? " (baseline: real full-frame re-encode)\n"
                : " (baseline: area-scaled estimate)\n");
        sb.append(String.format(Locale.US, "  %12s  %s%n", "VALUE", "METRIC"));
        sb.append(String.format(Locale.US, "  %12s  %s%n", "------------", "------"));
        if (uartBytesPerMs <= 0.0) {
            sb.append(
                    String.format(
                            Locale.US,
                            "  %12s  %s%n",
                            "n/a",
                            "UART TX speed unavailable — cannot judge crop time win"));
            sb.append(
                    String.format(
                            Locale.US,
                            "  %12s  %s",
                            "UNKNOWN",
                            "verdict: need UART speed to compare crop cost vs TX savings"));
            return;
        }

        // WITH crop (all measured): detect+decode+crop, then resize/sharpen/encode, then real TX.
        long actualTxMs = uart.durationMs;
        long withCropTotalMs = cropMs + compressMs + actualTxMs;
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(cropMs),
                        "with crop: detect + decode + crop"));
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(compressMs),
                        "with crop: resize/sharpen/encode"));
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(actualTxMs),
                        "with crop: BLE TX of " + formatKb(payloadBytes) + " (measured)"));
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(withCropTotalMs),
                        "with crop: post-capture total"));

        // WITHOUT crop: measured background re-encode of the same frame when available.
        long baselineProcessMs =
                reduction.measuredFullFrameProcessMs() > 0
                        ? reduction.measuredFullFrameProcessMs()
                        : compressMs;
        long baselineTxMs = Math.round(baselineBytes / uartBytesPerMs);
        long withoutCropTotalMs = baselineProcessMs + baselineTxMs;
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(baselineProcessMs),
                        "no crop: decode/resize/sharpen/encode ("
                                + (reduction.measuredFullFrameProcessMs() > 0
                                        ? "measured"
                                        : "assumed = crop path")
                                + ")"));
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(baselineTxMs),
                        "no crop: BLE TX of "
                                + formatKb(baselineBytes)
                                + " ("
                                + baselineTag
                                + ", at "
                                + String.format(
                                        Locale.US, "%.1fKB/s", uartBytesPerMs * 1000.0 / 1024.0)
                                + ")"));
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(withoutCropTotalMs),
                        "no crop: post-capture total"));

        long netMs = withoutCropTotalMs - withCropTotalMs;
        sb.append(
                String.format(
                        Locale.US,
                        "  %12s  %s%n",
                        formatMs(netMs),
                        "net time saved by cropping (no-crop total − crop total)"));
        String verdict;
        if (!reduction.cropApplied || cropSavedBytes <= 0) {
            verdict = "LOSS — crop ran but saved no BLE payload bytes";
        } else if (netMs > 50) {
            verdict = "WIN — cropping saved wall-clock time";
        } else if (netMs < -50) {
            verdict = "LOSS — crop cost more time than TX savings";
        } else {
            verdict = "NEUTRAL — crop cost ≈ TX time savings";
        }
        sb.append(
                String.format(
                        Locale.US, "  %12s  %s", verdict.split(" ", 2)[0], "verdict: " + verdict));
    }
