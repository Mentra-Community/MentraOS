...
public class K900CommandHandler {
        ...
        if (button == 0 && type == 0) {
            ...
            if (context != null) {
                ...
                if (powerManager != null && !powerManager.isInteractive()) {
                    ...
                    Log.d(
                            TAG,
                            "🔋 Device is asleep, acquiring short wake lock for battery"
                                    + " announcement");
    ...
    private void stopActiveRecordingBeforeShutdown() {
        try {
            ...
            if (mediaCaptureService != null && mediaCaptureService.isRecordingVideo()) {
                Log.i(
                        TAG,
                        "🎥 Active video recording detected - stopping before shutdown to prevent"
                                + " corruption");
    ...
    private void handlePhotoCapture(boolean isLongPress) {
        ...
        if (!isSaveInGalleryMode && isConnected) {
            Log.d(
                    TAG,
                    "📸 Camera app not active and connected to phone - skipping local capture"
                            + " (button press already forwarded to apps)");
            return;
        }
        ...
        if (!isConnected) {
            Log.d(
                    TAG,
                    "📸 Disconnected from phone - proceeding with local capture regardless of"
                            + " gallery mode");
        } else {
