...
/**
 * Interface for network management operations across different device types. This interface
 * abstracts WiFi and hotspot operations to support different implementations for different device
 * types (K900, system-level, fallback).
 */
public interface INetworkManager {
     ...
     * @return the strongest outcome the backend can honestly establish
     */
    WifiForgetOutcome forgetWifiNetwork(String ssid);

    /** Correlated saved-network listing protocol version, or 0 when unavailable. */
    default int getSavedWifiNetworksVersion() {
        return 0;
    }
    ...
    /** Reliable saved-network enumeration result for the public correlated protocol. */
    default SavedWifiNetworksResult getSavedWifiNetworksResult() {
        try {
            return SavedWifiNetworksResult.confirmed(getConfiguredWifiNetworks());
        } catch (UnsupportedOperationException e) {
            return SavedWifiNetworksResult.unsupported("list_saved_networks_unsupported");
        } catch (Exception e) {
