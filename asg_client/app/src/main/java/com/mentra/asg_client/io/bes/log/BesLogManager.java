 ...
 *
 * <p>When requested, sends {@code mh_logs} to BES and reassembles the streamed {@code sr_log}
 * response packets. On completion, uploads the assembled BES trace buffer to the incident backend
 * as source {@code "glasses_firmware"}.
 *
 * <p>Timeout chain:
 *
 * <ul>
 *   <li>2 s – first-packet timeout: BES may not support mh_logs in this build
 *   <li>20 s – overall safety timeout: if we never receive the terminator (cur=255, body=end), we
 *       stop waiting and upload whatever we have. Kept long so we don't cut off a stream that is
 *       still sending; only used when BES stalls or never sends terminator.
 * </ul>
 *
 * <p>One instance per collection request; not a singleton.
 */
public class BesLogManager {
    ...
    private static final long OVERALL_TIMEOUT_MS =
            20_000; // 20 s — only fires if terminator never arrives
    ...
    /**
     * Backend base URL for direct HTTP upload. When non-empty, takes precedence over {@link
     * com.mentra.asg_client.utils.ServerConfigUtil#getServerBaseUrl(android.content.Context)}.
     */
    private final String mApiBaseUrl;

    /** Create a new BES log collection session (HTTP upload on completion). */
    public BesLogManager(
            String incidentId, Context context, IConfigurationManager configurationManager) {
     ...
     * glasses' built-in server config. Falls back to {@link
    ...
    public static BesLogManager forRawLogCallback(
            Context context,
            IConfigurationManager configurationManager,
        ...
        mFirstPacketTimeout =
                () -> {
                    if (!mIsReceiving && !mFinished) {
                        Log.w(
                                TAG,
                                "⏰ No sr_log packet within 2 s — BES may not support mh_logs in"
                                        + " this build");
        ...
        mOverallTimeout =
                () -> {
                    if (!mFinished) {
                        Log.w(
                                TAG,
                                "⏰ Overall safety timeout (terminator not received) — uploading"
                                        + " partial ("
                                        + mLogBuffer.length()
                                        + " chars)");
    ...
    public void onLogPacketReceived(int cur, String body) {
        ...
        if (cur == TERMINATOR_CUR && TERMINATOR_BODY.equals(body)) {
            Log.i(
                    TAG,
                    "✅ BES log stream complete (cur=255, body=end) — "
                            + mLogBuffer.length()
                            + " chars collected");
    ...
    private void finish(String timeoutReason) {
        ...
        if (timeoutReason != null) {
            Log.w(
                    TAG,
                    "⚠️ BES log collection ended due to: "
                            + timeoutReason
                            + " ("
                            + fullLog.length()
                            + " chars)");
    ...
    private void uploadLogs(String logText) {
        try {
            ...
            Log.i(
                    TAG,
                    "[LOGS] Glasses firmware (BES) full request: method=POST url="
                            + url
                            + " body.source=glasses_firmware body.logs.length="
                            + logCount
                            + " bodyPreview="
                    ...
                    .enqueue(
                            new Callback() {
                                ...
                                public void onFailure(Call call, IOException e) {
                                    Log.e(
                                            TAG,
                                            "Failed to upload BES logs for incident " + mIncidentId,
                                            e);
                                }
                                ...
                                public void onResponse(Call call, Response response) {
                                    if (response.isSuccessful()) {
                                        Log.i(
                                                TAG,
                                                "✅ BES (glasses_firmware) logs uploaded for"
                                                        + " incident "
                                                        + mIncidentId
                                                        + " ("
                                                        + logCount
                                        ...
                                        Log.e(
                                                TAG,
                                                "❌ Server rejected BES logs upload — status: "
                                                        + response.code()
                                                        + " for incident "
                                                        + mIncidentId);
                                    }
