...
public class AsgClientService extends Service
  ...
  */
  ...
  /**
   * Send RGB LED control authority command to BES chipset. This tells BES whether MTK (our app) or
   * BES should control the RGB LEDs.
   *
  ...
  private void applySavedCameraFovOnStart() {
    try {
      ...
      try {
        ...
        Log.d(
            TAG, "Applied saved camera FOV on start: fov=" + fov + ", roi_position=" + roiPosition);
  ...
  public void onConnectionStateChanged(boolean connected) {
    Log.i(
        TAG,
        "📶 Bluetooth connection state changed: " + (connected ? "CONNECTED" : "DISCONNECTED"));
   ...
   * (version_info_2): ota_version_url, firmware_version, bt_mac_address Phone will accumulate both
   * chunks and process when complete.
   */
  public void sendVersionInfo() {
    ...
    try {
      ...
      if (serviceContainer.getServiceManager() != null
        ...
        besFirmwareVersion =
            serviceContainer.getServiceManager().getAsgSettings().getBesFirmwareVersion();
      }
      ...
      if (serviceContainer.getServiceManager().getBluetoothManager() != null
        ...
        Log.w(
            TAG, "⚠️ Bluetooth manager not available or not connected - cannot send version info");
      }
    } catch (JSONException e) {
    ...
    return new ServiceCallbackInterface() {
      ...
      public boolean sendFileViaBluetooth(String filePath) {
        ...
        if (serviceContainer.getServiceManager().getBluetoothManager() != null) {
          ...
          boolean started =
              serviceContainer.getServiceManager().getBluetoothManager().sendImageFile(filePath);
          if (started) {
      ...
      public boolean isBleTransferInProgress() {
        ...
        if (serviceContainer.getServiceManager().getBluetoothManager() != null) {
          boolean inProgress =
              serviceContainer.getServiceManager().getBluetoothManager().isFileTransferInProgress();
    ...
    try {
      ...
      for (String cameraId : cameraIds) {
        try {
          ...
          Log.i(
              TAG,
              "📹 Camera " + cameraId + " supports " + videoSizes.length + " video resolutions:");
  ...
  private void cleanupOrphanedBleTransfers() {
    try {
      ...
      Log.d(
          TAG, "🗑️ Checking for orphaned BLE transfer files in: " + appFilesDir.getAbsolutePath());
      ...
      for (java.io.File packageDir : packageDirs) {
        ...
        if (files != null && files.length > 0) {
          Log.d(
              TAG, "🗑️ Found " + files.length + " orphaned BLE files in " + packageDir.getName());
