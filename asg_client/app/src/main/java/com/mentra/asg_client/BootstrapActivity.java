...
/**
 * A transparent activity that starts the AsgClientService as a foreground service and then finishes
 * itself. This is used to launch the service after boot.
 */
public class BootstrapActivity extends Activity {
    ...
    private static final int FINISH_DELAY_MS =
            2000; // 2 second delay after starting before finishing
        ...
        try {
            ...
            PowerManager.WakeLock wakeLock =
                    powerManager.newWakeLock(
                            PowerManager.PARTIAL_WAKE_LOCK, "AugmentOS:BootstrapWakeLock");
                    ...
                    .postDelayed(
                            () -> {
                                try {
                                    Log.e(
                                            TAG,
                                            "BootstrapActivity delayed execution - starting"
                                                    + " AsgClientService now");

                                    // Start AsgClientService as a foreground service
                                    Intent serviceIntent = new Intent(this, AsgClientService.class);
                                    serviceIntent.setAction(
                                            AsgClientService.ACTION_START_FOREGROUND_SERVICE);
                                    ...
                                    // Wait a moment to ensure the service starts properly, then
                                    // finish
                                    new Handler(Looper.getMainLooper())
                                            .postDelayed(
                                                    () -> {
                                    ...
                                    Log.e(
                                            TAG,
                                            "ERROR starting AsgClientService: " + e.getMessage(),
                                            e);
