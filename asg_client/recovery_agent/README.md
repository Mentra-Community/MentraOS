# Mentra Recovery Agent

`recovery_agent` is a headless Android sidecar that keeps `com.mentra.asg_client` alive.
...
```bash
cd asg_client/recovery_agent
./build_and_deploy.sh
```

This builds `app-release.apk` and copies it to:

- `asg_client/app/src/main/assets/recovery_agent.apk`

The ASG app deploys this asset through `RecoveryAgentManager`.
