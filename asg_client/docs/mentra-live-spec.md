...
- The front-facing MTK privacy light turns on when a photo request enters the camera pipeline and turns off when the completed JPEG reaches `ImageReader`, with final-completion and error callbacks as idempotent fallbacks and a 45-second safety lease for lost callbacks. Queued photo requests share ownership so one completed photo cannot turn the light off while another is still pending.
