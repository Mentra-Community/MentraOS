...
Before BES authorization, ASG validates the immutable release identity, compressed hash and size, complete LZMA container and embedded CRC, decompressed hash and size, product marker, and embedded target version. The decompressed image must remain strictly below the BES bootloader's `0x1E0000`-byte flash limit. ASG atomically persists the stable Linux boot id and exact target version before the first `mh_ota` byte, and authorization may be attempted only once per glasses boot. A failed UART write is ambiguous: ASG retains exclusive OTA ownership and waits for `hm_ota`. If that framed reply is missing, ASG makes one one-way promotion to raw routing and sends up to three read-only, idempotent `0x99` protocol queries. A `0x9a` response proves BES is already in OTA mode and continues the transfer; the same five probe bytes contain no `##` marker and are harmless junk to the normal K900 parser. If all raw probes are silent, ASG never returns to framed routing and never resends `mh_ota`; it rejects both parsers and every UART write until a full glasses reboot. The persisted reservation restores that quarantine after a serial close/reopen or ASG process restart. After BES accepts apply, ASG persists a post-apply phase and the active phone OTA session, then distinguishes the authorization boot from the first subsequent Linux boot caused by BES's whole-glasses reset. Rediscovery is restricted to the system-version probe on that one verification boot; another reboot without proof quarantines the UART. The apply acknowledgement and direct `sr_adota` success/100% notification are nonterminal: ASG reports `FINISHED` only after the verification boot returns an exact target-version match in a fresh `sr_syvr`. ASG persists and retries that existing compact `ota_status` terminal snapshot after UART and phone readiness recover. A mismatch, verification timeout, or durable-state failure reports `install_failed`. The Mentra App conservatively treats any failed or locally timed-out BES install as restart-required, presents manual restart instructions, and withholds Retry until the user leaves that failure screen; detailed failure distinctions remain in ASG logs.
