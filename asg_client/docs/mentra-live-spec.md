...
Legacy v1 JSON frames sent through BES are capped at a fixed 240-byte packed envelope. `wire_caps.notify_cap` is diagnostic only because it may describe a higher-capacity EATT bearer without guaranteeing that every notification will use that bearer. Terminal OTA success/failure is therefore sent as one compact notification; the state-rich diagnostic remains in the device logs.

Before BES authorization, ASG validates the immutable release identity, compressed hash and size, complete LZMA container and embedded CRC, decompressed hash and size, product marker, and embedded target version. The decompressed image must remain strictly below the BES bootloader's `0x1E0000`-byte flash limit. ASG durably admits the phone OTA session, then atomically persists that session id with the stable Linux boot id and exact target version before the first `mh_ota` byte; authorization may be attempted only once per glasses boot. A failed UART write is ambiguous: ASG retains exclusive OTA ownership and waits for `hm_ota`. If that framed reply is missing, ASG makes one one-way promotion to raw routing and sends up to three read-only, idempotent `0x99` protocol queries. A `0x9a` response proves BES is already in OTA mode and continues the transfer; the same five probe bytes contain no `##` marker and are harmless junk to the normal K900 parser. If all raw probes are silent, ASG never returns to framed routing and never resends `mh_ota`; it rejects both parsers and every UART write until a full glasses reboot. The persisted reservation restores that quarantine after a serial close/reopen or ASG process restart. After BES accepts apply, that same durable record enters a post-apply phase and keeps the active phone OTA session from expiring while the hardware power-cycles. ASG distinguishes the authorization boot from the first subsequent Linux verification boot; rediscovery is restricted to the system-version probe on that boot, and another reboot without proof atomically records `install_failed` while quarantining the UART. The apply acknowledgement and direct `sr_adota` success/100% notification are nonterminal. A fresh `sr_syvr` exact match or mismatch atomically replaces the one apply record with a session-bound durable terminal handoff before any in-process event is emitted. All internal terminal events carry that same owner, and failure producers persist their owner-bound outcome before EventBus merely wakes a live consumer. `OtaService` rejects events whose owner is no longer current and reconciles the durable handoff under the same process-wide transition lock used for new-session admission. A durably admitted new session is the generation boundary and supersedes any older handoff by owner even if deleting its resend record fails, so service ordering, cleanup failure, or process restart cannot let one generation complete or fail another. Delayed resends stop as soon as their owning session is superseded. A verification timeout coordinates durable `install_failed` and in-memory UART quarantine under the UART monitor, excluding every probe and write from the transition. The Mentra App conservatively treats any failed or locally timed-out BES install as restart-required, presents manual restart instructions, and withholds Retry until the user leaves that failure screen; detailed failure distinctions remain in ASG logs.
