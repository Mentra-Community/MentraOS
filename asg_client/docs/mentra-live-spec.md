...
- The front-facing MTK privacy light turns on when a photo request enters the camera pipeline and turns off when the completed JPEG reaches `ImageReader`, with final-completion and error callbacks as idempotent fallbacks. K900 capture dispatch waits for the ordered LED-worker command to complete. A 45-second pre-exposure lease cancels the exact queued or active request if camera warmup stalls; sensor exposure replaces it with a lost-callback lease. The separate SDK photo-job timeout remains an additional terminal fallback. Photo capture, video recording, UVC, and network streaming share ownership of the privacy LED, so any active camera user keeps it on until all have finished.
