...
- The front-facing MTK privacy light turns on when a photo request enters the camera pipeline and turns off when the completed JPEG reaches `ImageReader`, with final-completion and error callbacks as idempotent fallbacks. A 45-second lost-callback safety lease starts only once sensor exposure begins, so time spent waiting in the photo queue cannot expire the light early. Photo capture, video recording, UVC, and network streaming share ownership of the privacy LED, so any active camera user keeps it on until all have finished.
