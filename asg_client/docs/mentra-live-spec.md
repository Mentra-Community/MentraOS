...
Before BES authorization, ASG validates the immutable release identity, compressed hash and size, complete LZMA container and embedded CRC, decompressed hash and size, product marker, and embedded target version. The decompressed image must remain strictly below the BES bootloader's `0x1E0000`-byte flash limit. Authorization is persisted before the first `mh_ota` byte and may be attempted only once per glasses boot. A failed UART write is ambiguous: ASG retains exclusive OTA ownership and waits for `hm_ota`. If that framed reply is missing, ASG promotes its inbound route and sends up to three read-only, idempotent raw `0x99` protocol queries. A `0x9a` response proves BES is already in OTA mode and continues the transfer; the same five probe bytes contain no `##` marker and are harmless junk to the normal K900 parser. Only silence from both the framed reply and all raw probes requires a glasses reboot and fresh BES version discovery. ASG reports that terminal state with the stable `bes_reboot_required` code when transport still permits it. The Mentra App also treats a local watchdog timeout during BES installation as reboot-required, presents manual restart instructions, and withholds Retry until the user leaves that failure screen. A definite authorization denial occurs before BES enters raw mode and remains retryable.
