...
The recovery agent is a headless companion APK (`com.mentra.recovery`) that keeps ASG alive when the main service crashes or fails to restart after an APK install.
...
The recovery agent validates that backup APK package name is `com.mentra.asg_client` before reinstall.

## Integration points

- ASG deploy/start manager: `RecoveryAgentManager`
