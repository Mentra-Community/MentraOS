...
6. **Verify after the glasses power cycle.** The `0x93` apply acknowledgement is not success. BES apply triggers the hardware's whole-glasses power cycle, so ASG records the authorization boot separately from the first subsequent verification boot. The exact target and active phone OTA session survive that boot transition and ASG process restarts on the verification boot. Only a fresh `sr_syvr` from that one claimed boot can produce `FINISHED`; a second reboot without proof atomically records `FAILED` and quarantines the UART.
7. **Deliver one verified terminal state.** BES sends `sr_adota` directly to the phone while UART is owned by the transfer, but its success/100% value means only that apply was accepted. The phone keeps that value nonterminal. Every internal terminal event carries its owning OTA session id, and failures are persisted by the producer before EventBus is used only as a wake-up signal. The boot-B verifier atomically replaces its one apply record with a session-bound durable terminal handoff before notifying in-process consumers. `OtaService` reconciles and queues that handoff under the same process-wide transition lock used for new-session admission, then resends only while its original session remains current. A durably admitted new session synchronously supersedes an older handoff by owner even if deleting that resend record fails; delayed events, cleanup, and resends whose owner is no longer current are ignored. A mismatch, timeout, second unverified reboot, or durable-state failure reports `FAILED`; it never relies on correcting an earlier false success.
