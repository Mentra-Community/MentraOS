...
7. **Deliver one verified terminal state.** BES sends `sr_adota` directly to the phone while UART is owned by the transfer, but its success/100% value means only that apply was accepted. The phone keeps that value nonterminal. The boot-B verifier atomically replaces its one apply record with a session-bound durable terminal handoff before notifying in-process consumers. `OtaService` reconciles and queues that handoff under the same process-wide transition lock used for new-session admission, then resends only while its original session remains current. A newer durably admitted session supersedes the older handoff by id, so a delayed replay cannot complete or fail the new update. A mismatch, timeout, second unverified reboot, or durable-state failure reports `FAILED`; it never relies on correcting an earlier false success.
