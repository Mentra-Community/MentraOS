...
7. **Deliver one verified terminal state.** BES sends `sr_adota` directly to the phone while UART is owned by the transfer, but its success/100% value means only that apply was accepted. The phone keeps that value nonterminal. Every internal terminal event carries its owning OTA session id, and failures are persisted by the producer before EventBus is used only as a wake-up signal. The boot-B verifier atomically replaces its one apply record with a session-bound durable terminal handoff before notifying in-process consumers. `OtaService` reconciles and queues that handoff under the same process-wide transition lock used for new-session admission, then resends only while its original session remains current. A new session is refused if an older mismatched handoff cannot be retired, and delayed events or resends whose owner is no longer current are ignored. A mismatch, timeout, second unverified reboot, or durable-state failure reports `FAILED`; it never relies on correcting an earlier false success.
