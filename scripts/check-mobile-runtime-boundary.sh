...
rg -l \
  'from ["'\'']@/stores/glasses|@/stores/gallerySync|@/stores/settings|@/stores/bluetoothSettingKeys|@/stores/display|useGlassesStore|useGallerySyncStore|useSettingsStore|useDisplayStore|waitForGlassesState|getGlasesInfoPartial|selectGlassesConnected|selectGlassesReady|toolkit\.stores\.glasses|toolkit\.stores\.gallerySync' \
  mobile/src \
  mobile/assets \
  -g '*.ts' \
  -g '*.tsx' \
...
# settings + display graduated to FAILING patterns above (slices 2-3); the
# allowlisted Cloud V1 files are their only sanctioned users until tier 5.
report_count "raw island stores via /internal + shims (core/connection/cloudClientStatus/appStatus)" \
  'useCoreStore|useConnectionStore|useCloudClientStatusStore|useAppStatusStore'
