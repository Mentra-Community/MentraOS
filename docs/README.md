...
Keep `docs/mentra-live/` self-contained so changes can be transplanted onto `frozen-docs` with minimal conflicts. App Dev pages should link to `/mentra-live/overview` when camera, photo, streaming, or direct mobile Bluetooth control is the right path.
