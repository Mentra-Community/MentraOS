...
if (session.capabilities?.hasCamera) {
  session.logger.info('Camera hardware is available');
}

// ❌ Avoid - assuming every glasses model has the same hardware
this.enableEveryHardwareFeature();
```
...
<CardGroup cols={2}>
  ...
  <Card title="Camera Glasses" icon="camera" href="/app-devs/core-concepts/hardware-capabilities/camera-glasses">
    Mentra Live hardware capabilities
  </Card>
