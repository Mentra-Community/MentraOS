...
When the glasses report the effective settings they used, stream status includes `resolvedConfig`.
This is separate from runtime `stats`: `resolvedConfig.video.frameRate`, `width`, `height`, and
`bitrate` describe the configured stream after defaults and glasses-side clamps.
`resolvedConfig.video.width` and `height` are the encoded output dimensions. When
present, `resolvedConfig.video.captureWidth` and `captureHeight` are the native
camera buffer dimensions selected by the glasses before center-crop or downscale;
they can match the output dimensions when no separate capture size is needed.
`resolvedConfig.audio` reports the audio bitrate, sample rate, echo cancellation,
and noise suppression settings used for the stream.
