...
| `exposureTimeNs` | `number` | `undefined` | Sensor exposure time in nanoseconds for this photo only. Omit for auto exposure. |
| `iso` | `number` | `undefined` | Sensor ISO for this photo only. Only used when `exposureTimeNs` enables manual exposure. |
| `timeout` | `number` | `5000` | Timeout in milliseconds |

Pass `exposureTimeNs` and `iso` together for manual shutter tuning. If `exposureTimeNs` is omitted, invalid, or unsupported by the connected glasses, the camera uses auto exposure and ignores `iso`.
