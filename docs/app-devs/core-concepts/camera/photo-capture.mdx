...
Pass `exposureTimeNs` for manual shutter tuning. If `exposureTimeNs` is omitted, invalid, or unsupported by the connected glasses, the camera uses auto exposure.
