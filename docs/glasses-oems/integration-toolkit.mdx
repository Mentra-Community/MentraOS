...
**Routing through your proxy.** You don't stand up a separate proxy service. When you configure the toolkit, you give it your proxy details and it routes all backend traffic through your infrastructure. Without a proxy, the toolkit talks to Mentra's backend services directly.
...
**Example: a display + mic device (no camera).** Most OEM glasses today are a single monochrome text display plus a microphone, no camera. Here's the whole adapter for one.

**Declare your capabilities.** Same capability shape the toolkit uses for every device it supports. Fill it in for your hardware. This device declares a display and a microphone, and leaves the rest out:
