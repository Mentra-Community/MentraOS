...
| LED, version, and OTA | `rgbLedControl`, `requestVersionInfo`, `setOtaVersionUrl`, `getOtaVersionUrl`, `checkForOtaUpdate`, `startOtaUpdate` |
...
| System | `requestVersionInfo`, `setOtaVersionUrl`, `getOtaVersionUrl`, `checkForOtaUpdate`, `startOtaUpdate` |
...
| API | Purpose |
| --- | --- |
| `setOtaVersionUrl(url)` | Configure the manifest URL used by both `checkForOtaUpdate()` and `startOtaUpdate()`. Pass `null` to restore the SDK default. |
| `getOtaVersionUrl()` | Return the currently configured manifest URL. |
