...
guard photo.response.state == .success else {
    throw PhotoError.requestRejected
}
```
...
`setCameraFov` accepts FOV degrees from 62 to 118 and ROI position `0` center, `1` bottom, or `2` top. The `"narrow"` (82°), `"standard"` (102°), and `"wide"` (118°) presets are also accepted and map to center ROI. On Mentra Live, applying FOV/ROI restarts the camera; the returned `SettingsAckEvent` resolves from the ASG client when the camera is ready again. Treat FOV as a framing/ROI control; output resolution and effective detail can vary by capture path, firmware, and camera mode.
