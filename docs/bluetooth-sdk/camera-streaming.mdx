...
export function StreamStatusLogger() {
  useBluetoothEvent('stream_status', (event) => {
    console.log(event);
    console.log(event.resolvedConfig?.video?.frameRate);
  });
...
override fun onStreamStatus(event: StreamStatusEvent) {
    ...
    Log.d("Mentra", "Resolved FPS: ${event.resolvedConfig?.video?.frameRate}")
}
```
...
func mentraBluetoothSDK(_ sdk: MentraBluetoothSDK, didReceive event: BluetoothEvent) {
    if case let .streamStatus(status) = event {
        ...
        print("Resolved FPS: \(status.resolvedConfig?.video?.frameRate ?? -1)")
    }
}
```
...
`frameRate` is the resolved capture/encode frame rate. `resolvedConfig.audio` reports
