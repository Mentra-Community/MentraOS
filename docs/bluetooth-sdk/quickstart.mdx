...
The Android artifact is published on Maven Central. Keep JitPack in `repositories` because the SDK depends on Android artifacts hosted there. The companion LC3 codec artifact resolves transitively as `com.mentraglass:lc3Lib`.

  </Tab>
  <Tab title="iOS">

Add the Swift package or CocoaPod.

For SwiftPM, add the `MentraBluetoothSDK` product. The package root is:
