...
- `setCameraFov({fov, roiPosition})` clamps `fov` to 62-118 degrees and `roiPosition` to `0` center, `1` bottom, or `2` top. On Mentra Live this persists the setting and restarts the camera for about 5 seconds before the next capture should run. Treat FOV as a framing/ROI control; output resolution and effective detail can vary by capture path, firmware, and camera mode.
