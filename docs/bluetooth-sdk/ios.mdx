...
Native iOS apps use the `MentraBluetoothSDK` Swift package or CocoaPod.
...
The Swift package product is `MentraBluetoothSDK`.
...
For a remote SwiftPM dependency, publish or tag `mobile/modules/bluetooth-sdk` as the root of the package repository, then add that repository URL in Xcode or `Package.swift`.
...
CocoaPods remains supported and is still the path used by MentraOS for optional native code that is not part of the core Swift package.
...
Use this when testing SDK source changes locally with CocoaPods:
