...
test("enterprise Runtime is release-matched and recorded by the dev coordinator", () => {
  ...
  assert.match(enterprise, /ghcr\.io\/mentra-community\/mentra-cloud/)
  assert.match(enterprise, /coreApiClientId:\{value:\$coreApiClientId\}/)
  assert.match(enterprise, /MENTRA_JWT_PRIVATE_KEY/)
  assert.match(enterprise, /source_digest.*image_digest|image_digest.*source_digest/s)
  assert.match(enterprise, /--arg workspaceHostname "enterprisedev\.mentraglass\.com"/)
