...
export function prepareMobileReleaseEnvironment({plan, template, backendEnvironment, otaManifestUrl, publicValues}) {
  ...
  const expectedBackend = plan.channel === "dev" ? "dev" : plan.channel === "beta" ? "staging" : null
  if (!expectedBackend)
    throw new Error(`Mobile prerelease builds do not support channel ${JSON.stringify(plan.channel)}`)
