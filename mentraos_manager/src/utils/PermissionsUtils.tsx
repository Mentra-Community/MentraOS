...
const PERMISSION_CONFIG: Record<string, PermissionConfig> = {
  [PermissionFeatures.BASIC]: {
    ...
    description: "Basic permissions required for MentraOS to function",
  ...
  [PermissionFeatures.POST_NOTIFICATIONS]: {
    name: "Notifications",
    description: "Allow MentraOS to send you notifications",
  ...
  [PermissionFeatures.READ_NOTIFICATIONS]: {
    ...
    description: "Allow MentraOS to forward notifications to your glasses",
...
//   description: 'Allow MentraOS to run in the background without battery restrictions',
...
export const displayCriticalPermissionDeniedWarning = (permissionName: string): Promise<boolean> => {
  return new Promise(resolve => {
    Alert.alert(
      ...
      `MentraOS needs ${permissionName.toLowerCase()} permissions to function properly. Please grant these permissions to continue.`,
      [
        {
          text: "Try Again",
          style: "default",
...
// Required for MentraOS Core permissions (now handled directly in React Native)
export const requestMentraOSPermissions = async (): Promise<boolean> => {
