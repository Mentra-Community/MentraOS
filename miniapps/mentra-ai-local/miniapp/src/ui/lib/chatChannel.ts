...
/**
 * Dev-only: ask a question by text. Routes through the same background
 * QueryProcessor pipeline as a spoken query; the answer arrives over the
 * regular chat:event stream, so the caller doesn't handle the response here.
 */
export function askByText(query: string): Promise<void> {
  return mentra.request("chat:ask", {query})
}
