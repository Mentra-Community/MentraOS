...
The first signed archive attempt in run `35390149026` compiled through native sources but failed signing embedded `Turf.framework` with `errSecInternalComponent`. Added a real disposable-binary signing probe before compilation, kept the temporary job keychain unlocked as in release CI, and enabled full signed-archive diagnostics. The existing clean-cache retry cannot diagnose private-key access; no artifact from this run was qualified or installed.
