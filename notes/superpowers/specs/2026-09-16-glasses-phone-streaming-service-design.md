...
3. **The stream service implements the hotspot client, and hotspot restore ends at the
   network.** An adapter may establish its destination during an `attach` (Mentra Call joins
   the ACS meeting in whichever attach finds it not yet joined, after the hotspot is ready and
   before the glasses publish); the stream service never closes a destination, and an adapter
   error thrown from `attach` is wrapped as `adapter_attach_failed` with the original error as
   `cause`. Its `restore` binds the listener and starts the receiver, then returns: the
...
export class StreamError extends Error {
  ...
  /** For adapter_attach_failed: the error the adapter threw, unchanged, so the consumer can classify it. */
