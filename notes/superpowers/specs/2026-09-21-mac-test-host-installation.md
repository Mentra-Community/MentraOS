...
| --- | --- | --- |
| `Install.command` cannot be verified | Downloaded executable script is quarantined. | Invoke the trusted installer directly for automated runs. |
| `launch-ios-on-mac` cannot be verified | Downloaded helper is ad hoc signed without Developer ID/notarization. | Prefer a stable provisioned helper; for the lab, clear quarantine only on a verified selected artifact. |
...
or screen capture. Signing/notarizing the Mac helper does not replace the inner
...
and pinned-launcher interface include 20 focused unit tests. The
...
## Improvements to the build ZIP

- Publish coordinated-release Mac ZIPs and receipts tied to the same app archive
  and effective OTA manifest, reusing the PR packaging contract.
- Preserve app bundle ID, distribution identity and relevant entitlements across
  PR/release exports; include the registered test Mac in provisioning.
- Expose noninteractive installation using a trusted preinstalled helper. Do not
  require a new downloaded script and ad hoc native launcher at every build.
- For broadly distributed helper binaries, use Developer ID signing and
  notarization. Keep iOS app trust/provisioning requirements separate.
- Record archive, executable, JavaScript, packaged configuration and helper hashes;
  never silently rebuild the candidate during installation.
- Qualify first setup, two distinct build replacements, relaunch and reboot, with
  trust, provisioning and privacy failures classified separately.

ZIP improvements can remove repeated helper/script warnings. They cannot grant a
fresh user's Bluetooth permission. The demonstrated local solution combines
verified artifact installation with persistent one-time host authorization.
