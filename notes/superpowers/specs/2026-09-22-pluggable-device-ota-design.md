...
Live's `idle` status alone is not proof that admitted work ended: ASG acknowledges Start before fetching the manifest and creating its session. Release requires terminal status, the coordinator's verified legacy completion evidence, or a correlated read of ASG's existing activity diagnostics proving no admission, update worker or restart remains active. That read must match the current query and native observation with no pending Start; new progress, retries or reconnects invalidate it. Older firmware without those diagnostics still requires terminal/completion evidence. A fresh unowned idle observation remains safe; unresolved recovery stays retained rather than granting permission to replace the glasses.
