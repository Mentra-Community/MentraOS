...
Live's native adapter observes the existing protocol without adding SID-based status filters or requiring new activity diagnostics. ASG can acknowledge Start before fetching the manifest; a fresh failure can have no SID or reuse the previous session's SID. Forward these events to the existing coordinator and clock-recovery handler, preserving cache/BES-heartbeat update ordering. The adapter retains a pending native command until it settles, but otherwise follows the existing idle/complete/failed reports; Engine separately retains its execution lease and completion/retry policy. For Live, `safeToRelease` expresses this compatibility policy, not proof of cross-attempt wire correlation. Older delayed wire results remain a pre-existing protocol limitation, not something this refactor can reliably identify. An explicit firmware attempt-ID protocol is deferred outside this PR. Do not suppress legitimate errors or require a firmware update to preserve existing Live behavior.
