...
Extend the existing final notification to wait for the matching iOS publication when that lane is applicable. Align trigger paths with new helper files. Account for the iOS job's 90-minute build budget instead of inheriting the ASG-only 40-minute polling assumption. Handle failed, cancelled, skipped and superseded runs; never wait indefinitely for a path-filtered lane. Preserve deduplication and make partial readiness explicit for each artifact. Link the exact published iPhone IPA and Mac ZIP, installation instructions and iOS build logs in both Slack and the existing PR build comment. Only report both downloads as ready after both verify. Keep Android/ASG links and pinned firmware metadata.
