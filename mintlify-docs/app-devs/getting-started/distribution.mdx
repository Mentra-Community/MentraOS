...
Dev mode is not an installation: the computer and CLI must remain running, and
the Mentra App keeps only one dev miniapp at a time. Scan a new dev QR and it
replaces the previous one. Use a release install when you want to leave the
computer, retain the miniapp's name and icon, or test multiple miniapps together.
