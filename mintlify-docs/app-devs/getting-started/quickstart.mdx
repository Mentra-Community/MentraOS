...
[Bluetooth SDK Quickstart](/bluetooth-sdk/quickstart) instead.
</Info>
...
On Android you can skip the Wi-Fi requirement entirely: plug the phone in and
run `bun dev --usb`. The CLI sets up `adb reverse` and points the QR at the
phone's own loopback address, so guest networks and client isolation stop
mattering.
</Info>
