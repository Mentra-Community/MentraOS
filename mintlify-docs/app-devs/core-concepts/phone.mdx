...
session.phone.notifications.on((data) => {
  session.display.render([
    {type: "text", id: "msg", box: {x: 0, y: 0, w: 576, h: 288}, text: `${data.app}: ${data.title}`},
  ])
})
```
...
<Note>
  `onDismissed` is **Android only**. iOS does not expose dismiss callbacks to apps (an Apple privacy restriction), so
  subscribing on iOS succeeds but no events ever fire. The matching `notifications.on()` post-event works on both
  platforms.
</Note>
