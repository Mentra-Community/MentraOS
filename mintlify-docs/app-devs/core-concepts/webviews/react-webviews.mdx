...
| `useHistoryState` | `useHistoryState<T>(key, initialValue): [T, SetHistoryState<T>]` | `useState` backed by browser history, so the phone's back gesture undoes each change. Use it for tabs, dialogs, and screens that are not routes. See [Back navigation](/app-devs/core-concepts/webviews/back-navigation). |
...
<CardGroup cols={2}>
  ...
  <Card title="Back navigation" icon="arrow-left" href="/app-devs/core-concepts/webviews/back-navigation">
    Make the back gesture step through your screens and dialogs.
  </Card>
