...
|---|---|---|
| `{type: "text", box, text, style?, id?}` | `style`: `border`, `radius`, `overflow`, `breakMode`, `maxLines`, `textWindow`, `verticalAlign` | Wrapped text inside the box. |
...
The host wraps text using the connected glasses' font metrics. The box is the
available area; optional `style.maxLines` imposes an additional line limit.
...
on the omitted side. These controls require Mentra App 3.2.0 or newer.
...
session.display.render([{
  ...
  style: {maxLines: 3, textWindow: "end", breakMode: "word"},
}]);
```

Each call still supplies the complete content. The host does not append to
history or accumulate text behind an element id.

Optional `style.verticalAlign: "top" | "bottom"` places the line region inside
...
```typescript
const result = await session.display.render(elements, {includeTextLayout: true});
const caption = result.textLayout?.caption;
// caption.lines: visible {text, start, end} entries
// caption.lineStarts: source starts of all wrapped lines
// caption.capacity: effective line limit; caption.truncated: whether text was omitted
```

Offsets use JavaScript UTF-16 string indices into that element's supplied text;
