...
registerMiniapp((session) => {
  session.input.onButtonPress((press) => {
    session.display.showTextWall(`${press.buttonId}: ${press.pressType}`);
  });
});
```
...
```typescript
session.input.onButtonPress((press) => {
  if (press.pressType === "long") session.display.clear();
});
```
...
session.input.onTouch((touch) => {
  session.display.showTextWall(touch.kind);
});

// One gesture.
session.input.onTouch("single_tap", () => {
  session.display.showTextWall("Tap");
});
