...
- If you ship one package id through several lanes, set `analytics.environment` (or the native metadata equivalent) so store builds are separable from dev and staging builds; store detection (`app_install_source`) is automatic.
...
- The app pins one exact completed Mentra release identity rather than a moving package tag.
- The self-hosted OTA bundle is the exact bundle selected by the completed
  release record for the pinned SDK release.
- Mentra SDK analytics are disabled before native SDK startup.
- The imported OTA ZIP matches the SHA-256 recorded in the completed release record.
- The final `version.json` and every artifact are reachable from both the phone and Mentra Live.
- The complete update and SDK feature set are tested with public internet access blocked.
- Older inventory is factory- or legacy-bootstrapped to ASG build 39 or newer.
