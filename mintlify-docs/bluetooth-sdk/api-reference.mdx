...
| `bluetooth_sdk_glasses_identified` | Once per connection after the glasses report a valid manufacturing serial. Carries `glasses_device_id` (the hardware serial, not a user or phone id) and the glasses-side firmware versions of identified glasses. |
