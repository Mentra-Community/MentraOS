...
| Streaming | `startStream`, `startExternallyManagedStream`, `sendExternallyManagedStreamKeepAlive`, `stopStream` |
...
- `startExternallyManagedStream` is for advanced coordinators that already own heartbeat scheduling. It uses the same request and response types as `startStream`, but it does not start the SDK-owned keep-alive monitor. Call `sendExternallyManagedStreamKeepAlive({streamId, ackId})` periodically with the same stream ID and a unique ACK ID. Its promise confirms native dispatch, not receipt of the corresponding glasses ACK. Correlate responses with `addListener('keep_alive_ack', ...)`, matching both `streamId` and `ackId`. Prefer `startStream` unless the caller also owns ACK correlation, timeout handling, and teardown.
...
| `startExternallyManagedStream(...)` | `StreamStatusEvent` for the requested stream once `status` is `streaming`; no SDK keep-alive monitor is started. | `stream_status` error/reconnect failure, timeout, or send failure. |
| `sendExternallyManagedStreamKeepAlive(...)` | `void` after native dispatch. | Native dispatch failure. The promise does not wait for a glasses ACK. |
...
| `keep_alive_ack` | `KeepAliveAckEvent` | Glasses acknowledgement for an externally managed stream heartbeat. Match both `streamId` and `ackId` to the request. |
