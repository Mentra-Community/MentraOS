...
| `bluetooth_sdk_glasses_identified` | Once per connection after the glasses report a valid manufacturing serial. Carries `glasses_device_id` (the hardware serial, not a user or phone id) and the glasses-side firmware versions. |
...
store, or `other_store`). Apps that ship one package id through several lanes
can add `app_environment` through the `analytics.environment` plugin option, the
`com.mentra.bluetoothsdk.analytics.environment` Android metadata, or the
`MentraBluetoothSdkAnalyticsEnvironment` `Info.plist` key (lowercase
