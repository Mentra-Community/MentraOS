...
| `bluetooth_sdk_glasses_identified` | Once per connection after the glasses report a valid manufacturing serial, and once per new reporting day (`America/Los_Angeles`) while that connection stays up (`event_kind=glasses_heartbeat`). Carries `glasses_device_id` (the hardware serial, not a user or phone id) and the glasses-side firmware versions of identified glasses. |

Every event carries the host app's `app_version`, `app_build`, `app_build_type`
(`debug` / `release`), and `app_install_source` (`play_store`, `app_store`,
`testflight`, `adhoc_or_dev`, `simulator`, `sideload`, a named third-party
store, `other_store`, or `unknown` when the platform gave no usable evidence).
Apps that ship one package id through several lanes can add `app_environment`
through the `analytics.environment` plugin option, the
`com.mentra.bluetoothsdk.analytics.environment` Android metadata, or the
`MentraBluetoothSdkAnalyticsEnvironment` `Info.plist` key (trimmed and
lowercased; must start with a letter or digit, then `[a-z0-9_-]`, at most 32
characters). Mentra treats an install as production only when
`app_install_source` is `app_store` or `play_store`, `app_build_type` is
`release`, and, when the host declares a lane, `app_environment` is `prod`;
store installs without a declared lane are reported as an unclassified cohort.

Analytics delivery never blocks Bluetooth SDK behavior: events are submitted
asynchronously. A failed upload is kept in a small on-device queue (at most 100
events, 7 days) and retried on the next successful send or SDK start (payloads
PostHog rejects outright are dropped); each event carries its own id and
capture time, so retries never double count.
...
| `event_kind` | `sdk_started`, `glasses_connected`, or `glasses_identified`. |
| `app_version`, `app_build`, `app_build_type`, `app_install_source`, `app_environment` | Host build facts described above (`app_installer_package` on Android carries the raw installer package). |
| `fully_booted` | Connection and identification events; whether the connected status already reports fully booted. |
| `glasses_model`, `glasses_is_simulated` | Connection and identification events; the model is included only when the SDK has a non-empty value. |
| `glasses_device_id`, `glasses_device_id_type` | Identification event only; the glasses manufacturing serial (`manufacturing_serial`). |
| `glasses_firmware_version`, `glasses_bes_firmware_version`, `glasses_mtk_firmware_version`, `glasses_android_version`, `glasses_app_version`, `glasses_build_number` | Identification event only; included when known. |

Apart from the glasses manufacturing serial on the identification event, the
SDK does not upload BLE MAC addresses, CoreBluetooth identifiers, Bluetooth
device names, user ids, tokens, Wi-Fi credentials,
...
For an explanation when a scan completes empty, use the callback overload:
...
sdk.scan(
    ...
    callback = object : MentraBluetoothScanCallback() {
        ...
        override fun onDiagnostic(diagnostic: ScanDiagnostic) {
            showScanHint(diagnostic.message)
        }

        override fun onComplete(devices: List<Device>) {
            finishDevicePicker(devices)
        }
    },
)
```

`onDiagnostic` is advisory and runs before `onComplete`; it does not call
`onError`. The `device_connected_on_phone` hint means a device
matching the scanned model is already connected to this phone. Neither platform
identifies the owning app, so another app is a possible explanation, not a
confirmed cause. This check cannot see connections held by another phone.

Diagnostics are skipped for cancelled or nonempty scans, simulated devices,
known SDK connections or connection attempts, and unavailable Bluetooth access.
Existing `ScanCallback` implementations keep their original completion behavior;
implement `ScanDiagnosticCallback` or extend `MentraBluetoothScanCallback` to
receive hints. iOS uses CoreBluetooth service-filtered connected peripherals;
models without a supported service filter or a matching identity stay silent.
...
Optional empty-scan hints are also available on React Native and iOS:

```ts
await BluetoothSdk.scan(DeviceModels.MentraLive, {
  onResults: renderDevicePicker,
  onDiagnostic: hint => showScanHint(hint.message),
})
// React hooks expose the same advisory as useBluetoothScan().diagnostic
// or useMentraBluetooth().scan.diagnostic.
```

```swift
let session = try sdk.scan(
    model: .mentraLive,
    timeout: 10,
    onResults: renderDevicePicker,
    onDiagnostic: { hint in showScanHint(hint.message) }
)
```

The React Native promise still resolves to an empty device list; hints do not
reject the scan. Hosts using an older native binary simply omit the hint until
rebuilt with this SDK. Clear the displayed hint when retrying, changing models,
connecting, or receiving results. The Mentra App and Starter Kit examples show
these hints alongside their retry controls.
...
| Wi-Fi and hotspot | `requestWifiScan`, `getSavedWifiNetworks`, `sendWifiCredentials`, `forgetWifiNetwork`, `setHotspotState` |
...
React Native helper exports include `DeviceModels`, `WifiForgetResult`, `WifiForgetOutcome`, `SavedWifiNetworksResult`, `SavedWifiNetworksOutcome`, `isConnectedGlassesConnectionStatus`, `isReadyGlassesConnectionStatus`, `isBusyGlassesConnectionStatus`, `isConnectedWifiStatus`, and `isEnabledHotspotStatus`. The React subpath exports `useMentraBluetooth`, `useBluetoothScan`, and `useBluetoothEvent`.
...
| `getSavedWifiNetworks()`                                 | `SavedWifiNetworksResult`. `outcome: "confirmed"` includes exact saved SSIDs from a reliable enumeration backend. `outcome: "unsupported"` includes `networks: []` on K900 and legacy firmware. Whitespace is preserved so a returned SSID can be passed back to `forgetWifiNetwork`. Capability-unknown calls wait for `version_info_1`; negotiation and response share one 15-second deadline. Only protocol version 1 is supported.                                                                                                                                                                                                                                                                                                                                                                                                                       | Capability-negotiation timeout, request timeout, session reset/disconnect, send failure, or another saved-list request in flight. Glasses-reported `unsupported` and `failed` are typed results, not transport exceptions.                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `forgetWifiNetwork(ssid)`                                | `WifiForgetResult` with `outcome` of `confirmed`, `dispatched`, `not_found`, `unsupported`, `failed`, or `legacy_unverified`. Capability-unknown calls wait for `version_info_1`; negotiation and response share one 15-second deadline. A discovery timeout never selects legacy. Modern results are correlated by request ID, glasses session ID, protocol version, and exact requested SSID; `wifi_status_change` cannot settle them. K900 returns `dispatched`, meaning its asynchronous SystemUI broadcast was queued, not that removal was verified. `connected` is an optional diagnostic snapshot and is omitted, rather than fabricated as `false`, if ASG cannot read link state. Legacy firmware returns `legacy_unverified` immediately after accepted native dispatch, without a fabricated link snapshot. Transport correlation remains internal. This return type intentionally replaces the previous `WifiStatusChangeEvent`. | Empty SSID, capability-negotiation timeout, request timeout, session reset/disconnect, send failure, or another Wi-Fi status command in flight. Glasses-reported semantic failures are typed results.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `queryVideoRecordingStatus(requestId)` | `VideoRecordingStatusEvent` with `status: "recording_status"`, the current `data.recording` value, and elapsed `data.duration_ms` when available. | Disconnected glasses, timeout, send failure, or a duplicate video request ID. |
...
| `startOtaUpdate()` | `OtaStartAckEvent` from `ota_start_ack`. | Known glasses battery below 5% (`battery_low` before ack), start-ack timeout, send failure, or another OTA start in flight. |
...
Each Bluetooth SDK release has a matching Mentra Live glasses software release with the same version number. When your app checks for an update, the SDK uses the OTA manifest for its own release. For the current `{{release-version}}` software and complete installation instructions, see [Update Mentra Live](/bluetooth-sdk/software-update).

For React Native apps, use Mentra Engine instead of assembling these primitives
yourself. `MentraLiveOtaFlow` supplies the exact Mentra App pages, and
`useMentraLiveOta` supports custom presentation over the same tested
coordinator. See [Update Mentra Live](/bluetooth-sdk/software-update#add-the-ota-requirement-to-your-app).

The APIs below are low-level building blocks used by native Android/iOS apps
and by Mentra Engine. They are not a second React Native OTA coordinator.
...
| `queryOtaStatus()` | Query and return the active OTA session status after a reconnect or interrupted response. React Native only. |
...
| `battery_status` | `BatteryStatusEvent` | Battery update from glasses. On Android with Mentra Live, the event can include an optional opaque diagnostic `eventId`. Do not parse or display it. |
...
| `wifi_scan_result` | `WifiScanResultEvent` | Wi-Fi scan results. Intermediate events can have `scanComplete: false`; the final event has `scanComplete: true` and is what `requestWifiScan()` awaits. On Android with Mentra Live, each event can include an optional opaque diagnostic `eventId`. |
| `wifi_forget_result`              | `WifiForgetResultEvent`             | Discriminated raw result union. `mode: "modern"` is session-correlated and carries semantic `outcome`; K900 `outcome: "dispatched"` does not prove the asynchronous vendor receiver removed the credential. `mode: "legacy"` preserves an older frame's `dispatched` boolean without inventing missing correlation fields. In either mode, optional `connected` is omitted when the wire frame has no diagnostic link snapshot. Delivery is at least once; raw listeners should deduplicate modern retries by `(requestId, sid)`. Promise APIs ignore duplicates after the first matching terminal result. |
| `saved_wifi_networks`             | `SavedWifiNetworksEvent`            | Session-correlated saved-SSID result from a backend that advertised support. Delivery is at least once; raw listeners should deduplicate retries by `requestId` and `sid`. Promise APIs ignore duplicates after the first matching terminal result.                                                                                                                                                                                                                                                                                                                                                                        |
| `version_info`                    | `VersionInfoEvent`                  | Glasses version metadata response. `version_info_1` can include `wifiForgetResultVersion` and `savedWifiNetworksVersion`; absent/0 means unsupported for that glasses session. Also returned by `requestVersionInfo()`.                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `mic_health` | `MicHealthEvent` | Native LC3 processing detects a `sequence_gap` or `decode_failure`. Includes cumulative current-connection counters and last LC3/PCM receipt timestamps. |
...
For request/response commands, prefer the returned value from the method, such as the `PhotoSuccessResponseEvent` returned by React Native `requestPhoto(...)` or the operation-specific video result returned by React Native `startVideoRecording(...)` / `stopVideoRecording(...)` / `queryVideoRecordingStatus(...)`. `requestPhoto(...)` waits for the terminal `photo_response`: it resolves after the photo is captured and delivered to the webhook, and rejects if the glasses reject the request, the phone-side fallback upload fails, or the terminal response times out. `stopVideoRecording(...)` keeps the same end-to-end shape for video webhook stops: it waits for recording stop plus upload success, and rejects on upload failure. Photo capture/upload/Bluetooth fallback progress follows through `photo_status`; raw video upload completion arrives through `media_success` and `media_error` for listeners that need the glasses event stream directly. Use event listeners for ongoing hardware streams and status updates: for example, `useBluetoothEvent('mic_pcm', ...)` receives a `MicPcmEvent`. `MicPcmEvent` includes `sampleRate`, `bitsPerSample`, `channels`, `encoding`, and `voiceActivityDetectionEnabled`; `MicLc3Event` includes `sampleRate`, `channels`, `encoding`, `frameDurationMs`, `frameSizeBytes`, `bitrate`, `packetizedFromGlasses`, and `voiceActivityDetectionEnabled`. `mic_health` reports native sequence/decode failures that cannot be reconstructed from PCM and includes `sequenceGapEvents`, `decodeFailures`, `lastLc3ReceivedAt`, and `lastPcmProducedAt`. `speaking_status` is separate from microphone audio frames so apps can use continuous audio while still reacting to speech activity.
...
Android and iOS expose typed callbacks/delegate methods instead of the React Native string event API. Android uses `MentraBluetoothSdkListener` methods such as `onStateChanged`, `onGlassesChanged`, `onSdkStateChanged`, `onScanChanged`, `onDeviceDiscovered`, `onButtonPress`, `onSpeakingStatus`, `onPhotoResponse`, `onPhotoStatus`, `onMicPcm`, `onMicHealth`, `onStreamStatus`, and `onOtaStatus`. iOS uses `MentraBluetoothSDKDelegate` methods such as `mentraBluetoothSDK(_:didUpdate:)`, `mentraBluetoothSDK(_:didUpdateGlasses:)`, `mentraBluetoothSDK(_:didUpdateSdkState:)`, `mentraBluetoothSDK(_:didUpdateScan:)`, `mentraBluetoothSDK(_:didDiscover:)`, `mentraBluetoothSDK(_:didReceive:)`, `mentraBluetoothSDK(_:didReceiveMicPcm:)`, and `mentraBluetoothSDK(_:didReceiveMicLc3:)`; microphone health arrives through `didReceive` as `.micHealth`, while OTA install progress arrives as `.otaStartAck` or `.otaStatus`. Microphone audio callbacks use `MicPcmEvent` and `MicLc3Event` objects with the same metadata as React Native.
...
### Combined JavaScript and Native Logs

The React Native SDK automatically forwards Android and iOS SDK diagnostics to
the JavaScript console after import. Native entries are prefixed with
`[native:android]` or `[native:ios]` and appear alongside your app's JS logs.
The `log` event remains available for custom consumers; an additional listener
that calls `console.log` will duplicate the SDK's automatic output.

Credential-bearing messages are redacted before console output, and each native
message is capped at 16,384 characters. The Mentra App attaches this combined
