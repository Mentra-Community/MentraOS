...
| `forgetWifiNetwork(ssid)`                                | `WifiForgetResult` with `outcome` of `confirmed`, `dispatched`, `not_found`, `unsupported`, `failed`, or `legacy_unverified`. Capability-unknown calls wait for `version_info_1` under a bounded negotiation deadline, then start a separate response deadline on dispatch. A discovery timeout never selects legacy. Modern results are correlated by request ID, glasses session ID, protocol version, and exact requested SSID; `wifi_status_change` cannot settle them. K900 returns `dispatched`, meaning its asynchronous SystemUI broadcast was queued, not that removal was verified. `connected` is an optional diagnostic snapshot and is omitted, rather than fabricated as `false`, if ASG cannot read link state. Older firmware can return `legacy_unverified` from a matching status. This return type intentionally replaces the previous `WifiStatusChangeEvent`. | Empty SSID, capability-negotiation timeout, request timeout, session reset/disconnect, send failure, or another Wi-Fi status command in flight. Glasses-reported semantic failures are typed results.                                                                                                                                                                                                                                                                                                                                                                                                             |
