...
| `getSavedWifiNetworks()` | Exact saved SSID strings from glasses with a reliable configured-network enumeration API. Whitespace is preserved so a returned SSID can be passed back to `forgetWifiNetwork`. | `list_saved_networks_unsupported` on K900, whose vendor credential store has no response callback; request timeout on older glasses; send failure; or another saved-list request in flight. |
...
| `forgetWifiNetwork(ssid)` | Current `WifiStatusChangeEvent` snapshot after ASG dispatches the platform forget command. The exact correlated result gets a 750 ms priority window over matching legacy status; older glasses then resolve from `wifi_status_change`. On K900, `dispatched: true` means the asynchronous SystemUI broadcast was queued, not that credential removal was verified. | ASG dispatch failure, empty SSID, timeout, send failure, or another Wi-Fi status command in flight. |
...
| `wifi_forget_result` | `WifiForgetResultEvent` | Exact-ID ASG dispatch result for `forgetWifiNetwork`; K900 `dispatched: true` does not prove the asynchronous vendor receiver removed the credential. |
| `saved_wifi_networks` | `SavedWifiNetworksEvent` | Exact-ID saved SSID result or terminal capability/error code for `getSavedWifiNetworks()`. |
