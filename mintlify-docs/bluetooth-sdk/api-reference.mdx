...
`onError`. The Android-only `device_connected_on_phone` hint means a device
matching the scanned model is already GATT-connected to this phone. Android does
not identify the owning app, so another app is a possible explanation, not a
...
receive hints. The React Native and iOS scan APIs do not emit this diagnostic.
