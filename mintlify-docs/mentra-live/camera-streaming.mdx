...
| `zsl` | No-op for action-button presets. Physical camera-button photos always use the global ZSL default; these fields are not persisted. Use per-request `requestPhoto({ zsl })` instead. |
| `mfnr` | No-op for action-button presets. Physical camera-button photos always use the global MFNR default; these fields are not persisted. Use per-request `requestPhoto({ mfnr })` instead. |
