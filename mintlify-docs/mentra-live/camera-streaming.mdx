...
| `zslMfnr` | React Native, Android, iOS | Coupled ZSL preview buffering + MFNR still capture. Enabled by default; pass `false` for the standard single-frame pipeline. Forced off for text/scan/manual exposure. |
| `mfnr` | React Native, Android, iOS | **Deprecated.** Use `zslMfnr`. Both legacy `mfnr` and `zsl` must be `true` to enable the pair when `zslMfnr` is absent. |
| `zsl` | React Native, Android, iOS | **Deprecated.** Use `zslMfnr`. Both legacy `mfnr` and `zsl` must be `true` to enable the pair when `zslMfnr` is absent. |
...
| `zslMfnr` | Ignored for physical camera-button photos, which use coupled ZSL/MFNR by default. Per-request `zslMfnr: false` still opts out for SDK captures. |
| `mfnr` | **Deprecated.** Legacy input; both `mfnr` and `zsl` must be `true` to enable the coupled pair when `zslMfnr` is absent. |
| `zsl` | **Deprecated.** Legacy input; both `mfnr` and `zsl` must be `true` to enable the coupled pair when `zslMfnr` is absent. |
