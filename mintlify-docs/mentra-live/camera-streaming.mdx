...
| `zsl` | React Native, Android, iOS | ZSL preview/capture buffering. Enabled by default; pass `false` to disable. Forced off for text/scan/manual exposure. |
| `mfnr` | React Native, Android, iOS | Vendor multi-frame noise reduction on still capture. Enabled by default; pass `false` for the single-frame pipeline. Forced off for text/scan/manual exposure. MFNR arms ZSL buffering in preview even when `zsl` is false. |
...
| `zsl` | Ignored for physical camera-button photos, which use the global ZSL default. Per-request `zsl: false` still opts out for SDK captures. |
| `mfnr` | Ignored for physical camera-button photos, which use the global MFNR default. Per-request `mfnr: false` still opts out for SDK captures. |
