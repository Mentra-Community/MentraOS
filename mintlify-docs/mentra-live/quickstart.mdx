...
<Warning>
This quickstart uses Bluetooth SDK `{{release-version}}`. Before testing SDK features, [install the matching Mentra Live `{{release-version}}` glasses software](/mentra-live/software-update). Every SDK release requires the Mentra Live software release with the same version number.
</Warning>
...
Select version `{{release-version}}`, then add the `MentraBluetoothSDK` product to your app target.
