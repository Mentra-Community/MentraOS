...
Each Bluetooth SDK release has a matching Mentra Live glasses software release with the same version number. When your app checks for an update, the SDK uses the OTA manifest for its own release. For the current `{{release-version}}` software and complete installation instructions, see [Update Mentra Live](/mentra-live/software-update).
...
coordinator. See [Update Mentra Live](/mentra-live/software-update#add-the-ota-requirement-to-your-app).
