...
<Warning>
Bluetooth connection alone does not confirm version compatibility. Before testing SDK features, [update Mentra Live to the glasses software matching your SDK version](/mentra-live/software-update). An app using Bluetooth SDK `{{release-version}}` must use the matching Mentra Live `{{release-version}}` software release.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Air-Gapped Deployment" icon="shield" href="/mentra-live/air-gapped-deployment">
