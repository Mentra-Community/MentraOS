...
For coordinated releases, the exact SDK packages, hosted OTA manifest, portable
OTA bundle, and completed release record share one release identity on a dedicated
GitHub release page. See [Air-Gapped Bluetooth SDK Deployment](/mentra-live/air-gapped-deployment)
for the asset naming contract, checksum verification, and internal hosting flow.
...
Mentra Live rejects `startOtaUpdate()` before `ota_start_ack` when its known battery level is below 5% and emits `ota_status` with `status: "failed"` and `error_message: "battery_low"`. An unknown glasses battery remains fail-open. Keep an app-level battery check as the user-facing policy; the glasses check is defense in depth.
