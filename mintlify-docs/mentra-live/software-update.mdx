...
Prebuilt Starter Kit apps have an independent publication step. The latest available prebuilt Android app currently uses SDK `{{example-app-version}}`; it updates glasses to that version, not necessarily to `{{release-version}}`.
