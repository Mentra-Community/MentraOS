...
The examples include the required OTA flow for installing the Mentra Live glasses software that matches their Bluetooth SDK version. The React Native example owns a forkable `src/ota/` presentation layer over Mentra Engine's tested controller, so you can change its cosmetics without reimplementing OTA sequencing. For `{{release-version}}` integration instructions and the separately published `{{example-app-version}}` prebuilt example, see [Update Mentra Live](/mentra-live/software-update).
