...
export default function PermissionsPage() {
  ...
  const [record, setRecord] = useState<Record<string, unknown>>(
    () => session.permissions.getAll() as unknown as Record<string, unknown>,
  )
    ...
    <Shell>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6">
        <p className="mb-3 text-[13px] text-muted-foreground">
          Tracks <em>manifest-declared</em> permissions only. <code className="font-mono">.has()</code> returning true
          means the manifest declared it — <strong>not</strong> that the OS actually granted it. To detect grant state,
          observe whether subscriptions deliver events.
        </p>
        ...
        <Row emoji="🔔" label=".onUpdate(handler) — fires count" value={String(updateCount)} mono />
