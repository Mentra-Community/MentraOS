...
export default function TranscriptionPage() {
  ...
  const toggleMulti = () => {
    ...
    const langs = multiLangs
      .split(",")
      .map((l) => l.trim())
      .filter(Boolean)
    unsubsRef.current.multi = session.transcription.forLanguage(langs, (d) => setLast("multi", d.text, d.language))
    setActiveSlots((s) => ({...s, multi: true}))
    append(`forLanguage([${langs.join(", ")}]) started`)
  }

  const handleConfigure = () => {
    const languageHints = hints
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
    ...
    append(
      `configure({languageHints: [${languageHints.join(", ")}], vocabulary: [${vocabulary.join(", ")}], diarization: true})`,
    )
  }
    ...
    <Shell>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6">
        <p className="mb-3 text-[13px] text-muted-foreground">
          Each section exercises one method in isolation. Requires <code className="font-mono">MICROPHONE</code> in
          miniapp.json.
        </p>
        ...
        <Section emoji="🔤" title=".forLanguage(lang, handler)" subtitle="One language.">
          ...
          <Button className="mt-2" onClick={toggleSingle}>
            {activeSlots.single ? "Stop" : "Start"}
          </Button>
          <div className="mt-2 truncate text-sm">
        ...
        <Section
          ...
          <Button className="mt-2" onClick={toggleMulti}>
            {activeSlots.multi ? "Stop" : "Start"}
          </Button>
          <div className="mt-2 truncate text-sm">
        ...
        <Section
          ...
          <Label className="mt-2" htmlFor="vocab">
            vocabulary
          </Label>
          <Input id="vocab" value={vocab} onChange={(e) => setVocab(e.target.value)} />
          <Button className="mt-2" onClick={handleConfigure}>
            Apply config
          </Button>
        </Section>
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="mb-2 text-[12px] font-semibold uppercase tracking-wider text-muted-foreground">Log</div>
          {log.length === 0 ? (
