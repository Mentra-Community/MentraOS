...
export function startDevSidecar(options: DevServerOptions): {stop: () => void; port: number} {
  ...
  const server = Bun.serve<ClientWsData, undefined>({
    ...
    websocket: {
      open(ws) {
        sockets.add(ws)
        log(`${COLOR.dim}[__mentra_dev]${COLOR.reset} client connected (${ws.data.remoteAddress})`)
      },
      message(ws, message) {
        let parsed: Inbound
        try {
...
function listProjectFiles(rootDir: string): string[] {
  const MAX_DEPTH = 5
  const MAX_FILES = 500
  const EXCLUDED_DIRS = new Set(["node_modules", "dist", ".git", ".next", "build", ".cache", ".turbo"])
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        ...
        if (out.length >= MAX_FILES) {
          console.warn(`[__mentra_dev] file list truncated at ${MAX_FILES} entries — bundle cache may be incomplete`)
          return out
        }
      }
    }
