...
static const struct gpio_dt_spec stp513n_i2c_scl = GPIO_DT_SPEC_GET(DT_PATH(zephyr_user), stp513n_scl_gpios);
#else
...
#else
#define STP513N_RESET_GPIO_AVAILABLE 0
#endif

// I2C initialization state
static bool stp513n_i2c_initialized = false;
static bool stp513n_driver_initialized = false;

// Software I2C timing parameters
#define STP513N_SW_I2C_DELAY_US 6    /* I2C bit timing delay (µs) */
#define STP513N_SW_I2C_TIMEOUT 1000U /* ACK wait timeout (loops) */

#if STP513N_SOFT_I2C_AVAILABLE
/* --- Software I2C GPIO operations --- */
static int stp513n_sda_out(void)
{
    return gpio_pin_configure_dt(&stp513n_i2c_sda, GPIO_OUTPUT);
}

static int stp513n_sda_in(void)
{
    return gpio_pin_configure_dt(&stp513n_i2c_sda, GPIO_INPUT | GPIO_PULL_UP);
}

static int stp513n_scl_out(void)
{
    return gpio_pin_configure_dt(&stp513n_i2c_scl, GPIO_OUTPUT);
}

// Note: stp513n_scl_in is kept for potential future use in I2C clock stretching detection
__attribute__((unused)) static int stp513n_scl_in(void)
{
    return gpio_pin_configure_dt(&stp513n_i2c_scl, GPIO_INPUT | GPIO_PULL_UP);
}

static void stp513n_sda_high(void)
{
    gpio_pin_set_raw(stp513n_i2c_sda.port, stp513n_i2c_sda.pin, 1);
}

static void stp513n_sda_low(void)
{
    gpio_pin_set_raw(stp513n_i2c_sda.port, stp513n_i2c_sda.pin, 0);
}

static void stp513n_scl_high(void)
{
    gpio_pin_set_raw(stp513n_i2c_scl.port, stp513n_i2c_scl.pin, 1);
}

static void stp513n_scl_low(void)
{
    gpio_pin_set_raw(stp513n_i2c_scl.port, stp513n_i2c_scl.pin, 0);
}

static int stp513n_sda_read(void)
{
    return gpio_pin_get_raw(stp513n_i2c_sda.port, stp513n_i2c_sda.pin);
}

/* --- Software I2C protocol functions --- */
static void stp513n_i2c_start(void)
{
    stp513n_sda_high();
    stp513n_scl_high();
...
static void stp513n_i2c_stop(void)
{
    stp513n_sda_low();
    mos_busy_wait(STP513N_SW_I2C_DELAY_US);
    stp513n_scl_high();
...
static int stp513n_i2c_write_byte(uint8_t b)
{
    stp513n_sda_out();
    mos_busy_wait(STP513N_SW_I2C_DELAY_US);
    ...
    /* 9th clock for ACK */
    stp513n_scl_low();
    stp513n_sda_in(); /* Switch to input, wait for slave ACK */
    mos_busy_wait(STP513N_SW_I2C_DELAY_US);
    
    stp513n_scl_high();
    mos_busy_wait(STP513N_SW_I2C_DELAY_US / 2);
    
    uint32_t t = 0;
    while (stp513n_sda_read() && t++ < STP513N_SW_I2C_TIMEOUT)
    {
        mos_busy_wait(10);
    }
...
static int stp513n_i2c_read_byte(uint8_t *p, bool ack)
{
    uint8_t val = 0;
    stp513n_sda_in();
    mos_busy_wait(STP513N_SW_I2C_DELAY_US);
...
static int stp513n_sw_i2c_write(uint8_t addr, const uint8_t *data, size_t len)
{
    stp513n_i2c_start();
    
    /* Send device address + write bit */
    if (stp513n_i2c_write_byte((addr << 1) | 0) != 0)
    {
        stp513n_i2c_stop();
        return -EIO;
    }
    ...
    if (data != NULL)
    {
        ...
        {
            if (stp513n_i2c_write_byte(data[i]) != 0)
            {
                stp513n_i2c_stop();
                return -EIO;
            }
...
static int stp513n_sw_i2c_read(uint8_t addr, uint8_t *data, size_t len)
{
    stp513n_i2c_start();
    
    /* Send device address + read bit */
    if (stp513n_i2c_write_byte((addr << 1) | 1) != 0)
    {
        stp513n_i2c_stop();
        return -EIO;
    }
    ...
    for (size_t i = 0; i < len; i++)
    {
        ...
        if (stp513n_i2c_read_byte(&data[i], ack) != 0)
        {
            stp513n_i2c_stop();
            return -EIO;
        }
...
/**
 * @brief Initialize software I2C for STP513N
 * 
...
{
    return stp513n_driver_initialized;
}
 ...
 */
static int stp513n_gpio_init(void)
{
...
int stp513n_init(void)
{
    ...
    ret = stp513n_i2c_init();
    if (ret != 0)
    {
        return ret;
    }

    ret = stp513n_gpio_init();
    if (ret != 0)
    {
 ...
 */
void stp513n_reset(void)
{
#if STP513N_RESET_GPIO_AVAILABLE && STP513N_SOFT_I2C_AVAILABLE
...
int stp513n_soft_reset(void)
{
    ...
    reg_temp = stp513n_read_reg(0xC8);
    if (reg_temp == 0xFF)
    {
    ...
    reg_temp |= 0x02;
    int ret = stp513n_write_reg(0xC8, reg_temp);
    if (ret != 0)
    {
...
uint8_t stp513n_read_reg(uint8_t addr)
{
    ...
    int ret = stp513n_sw_i2c_write(STP513N_I2C_ADDR, &addr, 1);
    if (ret == 0)
    {
        // Small delay before read
        mos_busy_wait(100);
        // Then read the value
        ret = stp513n_sw_i2c_read(STP513N_I2C_ADDR, &value, 1);
    }
...
int stp513n_write_reg(uint8_t addr, uint8_t value)
{
    ...
    int ret = stp513n_sw_i2c_write(STP513N_I2C_ADDR, data, 2);
    if (ret != 0)
    {
...
int stp513n_write_eeprom(uint8_t addr, uint8_t data)
{
    ...
    stp513n_write_reg(0xE0, 0x20);
    
    // Write data to EEPROM address (mapped to register 0x80 + addr)
    int ret = stp513n_write_reg(addr + 0x80, data);
...
uint8_t stp513n_get_eeprom_status(void)
{
    ...
    return (stp513n_read_reg(0xE1) & 0x02);
}
...
uint8_t stp513n_connect(void)
{
    ...
    reg_temp |= 0x44;
    stp513n_write_reg(0xC8, reg_temp);
    stp513n_write_reg(0xC0, 0x00);
    stp513n_write_reg(0xC1, 0x01);
    ...
    reg_temp |= 0x80;
    stp513n_write_reg(0x18, reg_temp);
...
int stp513n_read_config(uint8_t *buffer, uint8_t length)
{
    ...
    for (int i = 0; i < length; i++)
    {
        buffer[i] = stp513n_read_eeprom(i);
    }
...
int stp513n_update_config(const uint8_t *config_buffer, uint8_t config_len)
{
    ...
    for (int i = 0; i < 64; i++)
    {
        eep_buffer[i] = stp513n_read_eeprom(i);
    }
    ...
    eep_config_update = stp513n_config_verify(config_to_verify, eep_buffer, 64);
    ...
    while (stp513n_get_eeprom_status())
    {
        k_sleep(K_MSEC(10));
        over_time_cnt++;
        if (over_time_cnt > 30)
    ...
    {
        stp513n_write_eeprom(i, config_buffer[i]);
...
int stp513n_scan_i2c(uint8_t *found_addr)
{
    ...
    {
        mos_busy_wait(100);
        ret = stp513n_sw_i2c_read(STP513N_I2C_ADDR, &test_value, 1);
    }
    
    if (ret == 0)
    {
        LOG_INF("✅ Device found at 0x%02X, register 0xF0 = 0x%02X", STP513N_I2C_ADDR, test_value);
        if (found_addr != NULL)
        {
            *found_addr = STP513N_I2C_ADDR;
        }
        return 0;
    }
    else
    {
        LOG_WRN("Device at 0x%02X not responding (error: %d)", STP513N_I2C_ADDR, ret);
    }
    ...
    for (uint8_t addr = 0x08; addr <= 0x77; addr++)
    {
        ...
        ret = stp513n_sw_i2c_write(addr, NULL, 0);
        if (ret == 0)
        {
