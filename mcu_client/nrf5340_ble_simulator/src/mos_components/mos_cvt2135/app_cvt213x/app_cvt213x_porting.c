 ...
 * @note
 ****************************************************************/
__attribute__((weak)) int app_cvt213x_i2c_init(void)
{
#if 0
#if I2C_HW_EN

    GPIOEDE |= BIT(6) | BIT(5);     // 数字
    GPIOEDIR |= BIT(6) | BIT(5);    // 输入
    GPIOEPU300 |= BIT(6) | BIT(5);  // 上拉300
    GPIOEFEN |= BIT(6) | BIT(5);    // 用于功能IO
    FUNCMCON2 = I2CMAP_PE6PE5;      // 映射IIC对应的IO口
...
#endif
#endif
    /* Default weak implementation: success. Real driver overrides this with a proper init that may return non-zero on
     * error. */
    return 0;
}
...
/* ----------------- Periodic timer to invoke app_cvt213x_count ----------------- */
static struct k_timer app_cvt213x_k_timer;

static void app_cvt213x_k_timer_handler(struct k_timer* timer)
{
    ARG_UNUSED(timer);
    /* Call count handler to accumulate timing */
    app_cvt213x_count();
}

void app_cvt213x_timer_init(void)
{
    /* Initialize and start a periodic timer with 10ms period */
    k_timer_init(&app_cvt213x_k_timer, app_cvt213x_k_timer_handler, NULL);
    k_timer_start(&app_cvt213x_k_timer, K_MSEC(10), K_MSEC(10));
}

/* ----------------- Event queue + worker thread to ensure app_cvt213x_thread runs in a thread context -----------------
 */
...
static void app_cvt213x_thread_entry(void* p1, void* p2, void* p3)
{
    ...
    while (1)
    {
        /* Wait for next event and call the handler in thread context */
        k_msgq_get(&app_cvt213x_msgq, &ev, K_FOREVER);
 ...
 * @note
 ****************************************************************/
// static void app_cvt213x_irq_callback(void)
// {
...
void app_cvt213x_irq_init(void)
{
    ...
    // CVT2135_IRQ_INIT();
    extern int cvt213x_hal_irq_init(void); // porting TODO: host platform peripheral driver
...
void app_cvt213x_scheduler_init(void)
{
    ...
    k_thread_create(&app_cvt213x_tid, app_cvt213x_stack, K_THREAD_STACK_SIZEOF(app_cvt213x_stack),
                    app_cvt213x_thread_entry, NULL, NULL, NULL, K_PRIO_PREEMPT(7), 0, K_NO_WAIT);
...
void app_cvt213x_scheduler_put_event(EARBUD_CVT213X_ID id)
{
    ...
    /* Push event into the message queue for the worker thread to process.
     * Use non-blocking put to avoid blocking caller context; drop if queue is full.
     */
    uint32_t ev = (uint32_t)id;
    int      rc = k_msgq_put(&app_cvt213x_msgq, &ev, K_NO_WAIT);
    if (rc != 0)
    {
        CVT213X_APP_LOG_W(1, "app_cvt213x_scheduler_put_event: msgq full, dropping event 0x%x", id);
    }
}
