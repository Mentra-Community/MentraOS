...
{
    return (((v >> 0) & 0xff) << 24) | (((v >> 8) & 0xff) << 16) | (((v >> 16) & 0xff) << 8)
           | (((v >> 24) & 0xff) << 0);
}
...
static uint8_t gx8002_download_bootimg_stage1(void)
{
    ...
    mos_gx8002_iic_write_data(MOS_GX_DATA_ADDR, wbuffer, 4);
    ...
    while (wsize < boot_header.stage1_size)
    {
        ...
        mos_gx8002_iic_write_data(MOS_GX_DATA_ADDR, wbuffer, len);

        wsize += len;
    }

    LOG_INF("vad download stage1 size: %d, waiting 0x46 ...", wsize);
    if (mos_gx8002_iic_wait_reply(MOS_GX_CMD_ADDR, 0xA4, 0x46, 5000) == 0)
    {
    ...
    wbuffer[0] = 0x59;
    if (mos_gx8002_iic_write_data(MOS_GX_DATA_ADDR, wbuffer, 1))
    {
        if (mos_gx8002_iic_wait_reply(MOS_GX_CMD_ADDR, 0xA0, 0x55, 1000))
        {
...
static uint8_t gx8002_download_bootimg_stage2(void)
{
    ...
    wbuffer[0] = 0xef;
    mos_gx8002_iic_write_data(MOS_GX_DATA_ADDR, wbuffer, 1);
    if (!mos_gx8002_iic_wait_reply(MOS_GX_CMD_ADDR, 0xa0, 0x78, 1000))
    {
    ...
    if (!mos_gx8002_iic_wait_reply(MOS_GX_CMD_ADDR, 0xa4, 0x46, 1000))
    {
...
static uint8_t gx8002_download_flashimg(void)
{
    ...
    wbuffer[2] = temp[2];
    wbuffer[3] = temp[3];
    if (!mos_gx8002_iic_write_data(MOS_GX_DATA_ADDR, wbuffer, 4))
    {
    ...
    if (!mos_gx8002_iic_wait_reply(MOS_GX_CMD_ADDR, 0xA4, 0x43, 10000))
    {
    ...
    int block_size = (UPGRADE_FLASH_BLOCK_SIZE < (int)fw_data_len) ? UPGRADE_FLASH_BLOCK_SIZE : (int)fw_data_len;
    ...
    while (wsize < img_size)
    {
        ...
        if (!mos_gx8002_iic_write_data(MOS_GX_DATA_ADDR, data_buf + (wsize % UPGRADE_FLASH_BLOCK_SIZE), len))
        {
        ...
        if ((wsize % UPGRADE_FLASH_BLOCK_SIZE) == 0 && wsize < img_size)
        {
            ...
            if (!mos_gx8002_iic_wait_reply(MOS_GX_CMD_ADDR, 0xa4, 0x44, 1000))
            {
            ...
            int next_block_size = (remaining > UPGRADE_FLASH_BLOCK_SIZE) ? UPGRADE_FLASH_BLOCK_SIZE : remaining;
            if (current_firmware_data == NULL || current_firmware_len == 0)
            {
                LOG_ERR("vad firmware data not set");
                return 0;
            ...
            if (read_offset >= 0 && read_offset < (int)fw_data_len)
            {
                ...
                int copy_len = (next_block_size < remaining_data) ? next_block_size : remaining_data;
...
uint8_t gx8002_fw_update(const uint8_t *firmware_data, uint32_t firmware_len)
{
    ...
    {
        LOG_WRN("vad handshake failed after reset, retry one more reset");
        mos_gx8002_reset();
        k_sleep(K_MSEC(50));
        if (!mos_gx8002_handshake())
        {
