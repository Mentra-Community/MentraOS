...
#if defined(CONFIG_LVGL)
    add_dynamic_font_label(g_main_scene.welcome.welcome_text);
    add_dynamic_font_label(g_main_scene.caption.default_scrolling);
#endif
    ...
    if (mos_binfont_get_current_language() != MOS_FONT_LANG_EN_US)
    {
        ...
        if (switch_ret != 0)
        {
            LOG_WRN("Force binfont to EN failed (lang=%u size=%u): %d",
                    (unsigned int)MOS_FONT_LANG_EN_US, (unsigned int)target_size, switch_ret);
        }
    }
#endif
