...
const logger = createLogger("store").child({service: "miniapp.service"})
...
export class MiniAppService {
  ...
  private async publishReleaseWithLease(input: AdminReleaseDecisionInput, lease: StoreListingLease) {
    ...
    const readiness = storeListingReadiness({description: app.description, storeListing: reviewedStoreListing}, assets)
    if (!readiness.ready) {
      throw new MiniAppServiceError(
        "store_listing_incomplete",
        `Store listing is incomplete: ${readiness.missing.join(", ")}`,
  ...
  async updateStoreModeration(input: AdminStoreModerationInput) {
    ...
    const target = await MiniAppModel.findOne({packageName, status: {$ne: "archived"}})
      .select("_id")
      .lean()
    ...
    return withStoreListingLease(authorizedApp._id.toString(), async (lease) => {
      ...
      try {
        ...
        if (input.role === "store_icon") {
          const updated = await MiniAppModel.updateOne(listingReferenceFilter(), {
...
export async function withStoreListingLease<T>(
  ...
  throw new MiniAppServiceError("store_listing_busy", "Store listing is busy; retry the operation", 409)
}
...
function storeListingAssetIds(listing: ReturnType<typeof serializeStoreListing>): Set<string> {
  return new Set(
    [listing.iconAssetId, listing.coverAssetId, ...listing.screenshotAssetIds].filter(
