...
export async function parseCanonicalBundleManifest(
  ...
  if (!PACKAGE_NAME_PATTERN.test(packageName) || packageName !== manifest.packageName) {
    throw new BundleManifestError("invalid_package_name", "miniapp.json packageName must be lowercase reverse-DNS text")
  }
...
function isSafeZipPath(path: string): boolean {
  ...
  return (
    normalized.length > 0 && normalized.split("/").every((piece) => piece !== "" && piece !== "." && piece !== "..")
  )
}
