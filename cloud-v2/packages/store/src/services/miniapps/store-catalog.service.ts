...
export class StoreCatalogService {
  ...
  async get(packageName: string, baseUrl: string, identity?: StoreCatalogIdentity) {
    ...
    const betaInstallAuthorizedAppIds = await this.betaAuthorizedAppIds(identity ?? {}, [app._id.toString()], true)
    ...
    const betaSelections = identity
      ? await this.betaSelectedAppIds(identity, publishedReleaseIds, betaInstallAuthorizedAppIds, [app._id.toString()])
      : new Set<string>()
    const betaAppIds = await this.betaEligibleAppIds(
      betaSelections,
      publishedReleaseIds,
    ...
    if (releaseTrack === "beta") {
      ...
      if (!authorized.has(app._id.toString())) {
        throw new StoreCatalogError(
          ...
          "this private beta requires a developer invitation",
          403,
        )
      }
  ...
  async getBundleAsset(assetId: string, identity: StoreCatalogIdentity) {
    ...
    if (release.releaseTrack === "beta") {
      const betaInvitationAuthorized = (await this.betaInvitationAuthorizedAppIds(identity, [appId], true)).has(appId)
      const betaDistributionAuthorized =
        app.visibility === "private"
          ? appInvitationAuthorized
          : app.betaAccessMode === "public"
  ...
  private async publicApprovedReleaseIds(): Promise<string[]> {
    return (await MiniAppReleaseModel.distinct("_id", {status: "published", publicStoreApprovedAt: {$ne: null}})).map(
      String,
    )
  }
  ...
  private catalogFilter(
    ...
    return {
      ...
      $and: [
        {
          $or: [{visibility: {$ne: "private"}}, {visibility: "private", _id: {$in: [...privateAuthorizedAppIds]}}],
        },
      ],
  ...
  private async serializeApps(
    ...
    return apps.flatMap((app) => {
      ...
      const privateDistribution =
        ...
        (selectedTrack === "beta" && (app.betaAccessMode !== "public" || transitionAuthorizedBetaAppIds.has(appId)))
      ...
      const hasPublishedStable = Boolean(
        app.publishedStoreListing &&
          ...
          (app.visibility === "private" || stableRelease?.publicStoreApprovedAt || privateAuthorizedAppIds.has(appId)),
      )
      ...
      return [
        {
          ...
          availableTracks: [...(hasPublishedStable ? ["stable"] : []), ...(canAccessBeta ? ["beta"] : [])],
          release: {
