...
import {DeveloperOrgMembershipModel, type DeveloperOrgRole} from "../../models/developer-org-membership.model"
...
export class DeveloperOrgService {
  async getPrimaryOrgForUser(user: ConsoleUserIdentity): Promise<DeveloperOrgRecord | null> {
    const org = await DeveloperOrgModel.findOne({ownerUserId: user.id}).sort({createdAt: 1}).lean()
  ...
  async listOrgsForUser(user: ConsoleUserIdentity): Promise<DeveloperOrgRecord[]> {
    ...
    const orgs = await DeveloperOrgModel.find({
      $or: [{ownerUserId: user.id}, ...(membershipOrgIds.length > 0 ? [{orgId: {$in: membershipOrgIds}}] : [])],
    })
  ...
  async listMembers(orgId: string): Promise<DeveloperOrgMemberRecord[]> {
    const rows = await DeveloperOrgMembershipModel.find({orgId}).sort({createdAt: 1}).lean<
      Array<{
        userId: string
        email?: string | null
        name?: string | null
