...
import {authenticateWorkosRequest} from "@mentra/developer-auth"
import {z} from "zod"
import {
  DeveloperOrgService,
...
app.get("/health", (c) => c.json({status: "ok", service: "miniapp-store-console"}))
    ...
    if (isWorkosRequestError(error)) {
      ...
      if (selection) {
        if (selection.organizations.length === 1) {
          return completeOrganizationSelection(c, {...selection, returnTo}, selection.organizations[0].id, "redirect")
        }
...
async function getTokens(c: AppContext) {
  ...
  try {
    return c.json({tokens: await apiKeys.list(developer.org.id, storeEnvironmentLabel())})
  } catch (error) {
    return serviceError(error)
  }
}
...
function workosOrgMetadata(org: DeveloperOrgRecord): Record<string, string> {
  return {
    ...
    storeEnvironment: storeEnvironmentLabel(),
  }
}

function storeEnvironmentLabel(): string {
  const explicit = process.env.MENTRA_STORE_ENVIRONMENT ?? process.env.CLOUD_CORE_ENVIRONMENT
...
export async function authenticateConsoleSession(c: AppContext): Promise<ConsoleAuthResult> {
  ...
  if (bearer?.startsWith("msk_")) return authenticateApiKeyToken(bearer)
  const config = workosConfig()
  const auth = await authenticateWorkosRequest(c, {
...
async function authenticateApiKeyToken(token: string): Promise<ConsoleAuthResult> {
  try {
    const validated = await apiKeys.validate(token, storeEnvironmentLabel())
...
function workosOrganizationSelection(error: unknown): WorkosOrganizationSelection | null {
  ...
  const organizations = rawOrganizations
    .map((org) => {
      ...
      const id =
        typeof value.id === "string" ? value.id : typeof value.organization_id === "string" ? value.organization_id : ""
      const name =
...
function workosConfig(): {
  ...
  const redirectUri = process.env.WORKOS_REDIRECT_URI || "http://localhost:3003/api/console/auth/callback"
