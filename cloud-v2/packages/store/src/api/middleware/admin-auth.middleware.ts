...
function isAdminEmail(email: string): boolean {
  ...
  const emails = parseList(process.env.MENTRA_STORE_ADMIN_EMAILS ?? process.env.CLOUD_CORE_ADMIN_EMAILS).map((value) =>
    value.toLowerCase(),
  )
  ...
  const domains = parseList(process.env.MENTRA_STORE_ADMIN_EMAIL_DOMAINS ?? process.env.CLOUD_CORE_ADMIN_EMAIL_DOMAINS)
  return domains
