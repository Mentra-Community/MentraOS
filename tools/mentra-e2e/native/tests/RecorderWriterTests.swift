...
struct DriverFailure: Error { let message: String; init(_ message: String) {
...
/// Keep AVFoundation's pre-Sendable track objects on the generic executor.
/// Only value-type observations cross back to async main's actor on older SDKs.
func readMovie(_ path: String) async throws -> (Double, [Double], [Int]) {
    ...
    return (duration, times, pixels)
}

@main struct RecorderWriterTests {
    static func main() async throws {
        let directory = CommandLine.arguments[1]
        let origin = CMTimeSubtract(CMClockGetTime(CMClockGetHostTimeClock()), CMTime(seconds: 8, preferredTimescale: 60000))
        func at(_ offset: Double) -> CMTime {
            CMTimeAdd(origin, CMTime(seconds: offset, preferredTimescale: 60000))
        }
        let path = directory + "/relaunch.mp4"
        let recorder = try RecordingObserver(path: path, width: 64, height: 64)
        try recorder.accept(frame(at: at(0), shade: 20), of: .screen)
        ...
        try recorder.accept(frame(at: at(3), shade: 90), of: .screen)
        ...
        let (duration, times, pixels) = try await readMovie(path)
        try require(abs(duration - 10) < 0.01, "MP4 omitted the post-reattach timeline or idle tail")
        try require(times.count == 3, "encoded frames were lost")
        ...
        try wrongSize.accept(frame(at: at(0), shade: 20, width: 32), of: .screen)
        ...
        try backwards.accept(frame(at: at(0), shade: 20), of: .screen)
        try backwards.accept(frame(at: at(0), shade: 30), of: .screen)
