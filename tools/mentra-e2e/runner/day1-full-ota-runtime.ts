...
export function createDay1FullOtaRuntime(
  ...
  async function command(bound: Day1FullOtaRuntimeContext, argv: string[], stdin?: string) {
    ...
    const cfg = JSON.parse(cfgBytes.toString("utf8"))
    if (!cfg.lease || Object.keys(cfg.lease).join("|") !== "path")
      throw new Error("January immutable lease configuration must contain only its path")
    ...
    if (lease.pid !== process.pid || typeof lease.token !== "string" || !lease.token)
      throw new Error("January Python must be spawned directly by the current fixture lease owner")
