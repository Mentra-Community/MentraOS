...
The experimental [connected Call replay](CONNECTED-CALL-REPLAY.md) now coordinates the native app, a validated machine fixture and the [Teams browser companion](TEAMS-BROWSER-ROUTINE.md). `connected-call.ts describe` prints its English routine; `validate` checks configuration without hardware access. Its `run` command covers incoming video, admission, roster and owned cleanup. It still needs a successful live qualification and does not establish two-way audio or native iPhone hotspot association.
