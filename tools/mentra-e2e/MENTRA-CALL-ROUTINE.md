...
24. Verify **Direct link for Teams is on** and the Mac has a working Ethernet default route while Wi-Fi remains enabled. Record system Location Services and Mentra's location access; the current join checks Wi-Fi SSID, and the first Mac denied that information with Location off. Resolve permission changes only with user approval. Stop before joining if the Mac still depends on Wi-Fi for internet. Capture every hotspot, local receiver and internet-route checklist stage. The updated native check recognizes Ethernet and cellular; its path result is advisory for ACS, whose actual connection must still succeed. Do not change to cloud relay or label a Mac hotspot pass as an iPhone SoftAP pass.
...
The app disabled the hotspot on teardown, returned home, and all three Mac audio defaults were restored. The operator retired this exact meeting after matching ID, subject and creation time (DELETE 204, GET 404); `meeting-cleanup.json` records this separately from app cleanup. Private setup/native logs are in `2026-09-16T23-09-37Z-mac-hotspot-setup`. macOS denied Wi-Fi information due to missing Location access despite correct signed entitlements. Global Location Services and Mentra access were both off; approval to enable them is pending. This explains missing SSID visibility, but association-error causality remains unproven.
