...
All three Mac hotspot meetings were retired separately after exact identity/time checks, and each attempt verified hotspot/audio cleanup. Location settings stayed off. No ACS or browser media pass exists. A single foreground retry is proposed to isolate application state; normal replay still preserves the user's foreground app, and the exception awaits approval. These diagnostic traces remain discovery evidence, not a qualified connected-call cache.
