...
```typescript
// TypeScript
const client = new LiveKitBridgeClient("localhost:9090", grpc.credentials.createInsecure(), {
...
export class LiveKitGrpcClient {
  ...
  async playUrl(params: {requestId: string; url: string; volume?: number}): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      stream.on("data", (event: any) => {
        if (event.type === "STARTED") {
          ...
          this.logger.info({requestId: params.requestId}, "Playback completed")
          resolve()
...
```typescript
const client = new LiveKitBridgeClient("unix:///tmp/livekit-bridge.sock", grpc.credentials.createInsecure())
```
