...
export class ClientAppsService {
  ...
  static async getAppsForHomeScreen(userId: string): Promise<AppletInterface[]> {
    try {
      ...
      logger.debug({userId, count: result.length}, "Fetched apps for home screen")
...
import {clientAuthWithEmail, RequestWithEmail} from "../middleware/client.middleware"
...
export type {AppletInterface, AppletPermission, AppletType, AppPermissionType} from "@mentra/types"
