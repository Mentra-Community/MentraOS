...
<Warning>
**Self-Hosting Note**: While the cloud can be self-hosted, real-time transcription requires third-party API keys (Soniox or Azure Speech). There is currently no fully offline transcription option.
</Warning>
...
# Transcription Services (at least one required - no offline option)
# Soniox is the default provider, Azure Speech is the fallback.

# Soniox Speech (Default provider)
...
# LLM Configuration (Currently Unused)
# NOTE: The LLM infrastructure exists but is NOT currently used by any features.
# You can leave these empty or set dummy values. Preserved for future AI features.
LLM_MODEL=gpt-4o
LLM_PROVIDER=azure

# Azure OpenAI (not currently used)
...
**Required for transcription (at least one):**
- `SONIOX_API_KEY` - Default transcription provider
- `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION` - Fallback provider
...
- LLM configuration (Azure OpenAI, OpenAI, Anthropic) - can be left empty
...
#### LLM Provider Setup (Currently Unused)

<Note>
The LLM provider infrastructure exists but is **not currently used** by any cloud features. You can skip this section entirely and leave the LLM variables empty. This is preserved for future AI features.
</Note>
