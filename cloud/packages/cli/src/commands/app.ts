...
appCommand
  ...
  .action(async (options) => {
    try {
      ...
      const description =
        options.description || (isInteractive ? await input("Description:", { required: false }) : "");
      ...
      const logoUrl =
        options.logoUrl || (isInteractive ? await input("Logo URL (optional):", { required: false }) : "");
      ...
      posthog.capture(creds.email, "app_created", {
        package_name: result.app.packageName,
        app_type: result.app.appType,
  ...
  .action(async (packageName: string, options) => {
    try {
      ...
      if (isInteractive) {
        ...
        posthog.capture(creds.email, "app_updated", { package_name: packageName, interactive: true });
        await posthog.shutdown();
        console.log("\nUpdated app details:");
        displayJSON(result);
      } else {
        ...
        const result = await api.updateApp(packageName, updateData);
        success(`App updated: ${result.packageName}`);
        posthog.capture(creds.email, "app_updated", { package_name: packageName, interactive: false });
        await posthog.shutdown();
        displayJSON(result);
      }
    } catch (err: any) {
...
appCommand
  ...
  .action(async (packageName: string, options) => {
    try {
      ...
      posthog.capture(creds.email, "app_deleted", { package_name: packageName, app_type: app.appType });
      await posthog.shutdown();
    } catch (err: any) {
      posthog.captureException(err);
      await posthog.shutdown();
...
appCommand
  ...
  .action(async (packageName: string, options) => {
    try {
      ...
      posthog.capture(creds.email, "app_published", {
        package_name: packageName,
        app_store_status: result.appStoreStatus,
      });
      await posthog.shutdown();
    } catch (err: any) {
      posthog.captureException(err);
...
appCommand
  ...
  .action(async (packageName: string, options) => {
    try {
      ...
      posthog.capture(creds.email, "app_api_key_regenerated", { package_name: packageName });
      await posthog.shutdown();
...
appCommand
  ...
  .action(async (packageName: string, options) => {
    try {
      ...
      posthog.capture(creds.email, "app_exported", {
        package_name: packageName,
...
appCommand
  ...
  .action(async (file: string, options) => {
    try {
      ...
      posthog.capture(creds.email, "app_imported", {
        package_name: result.app.packageName,
        app_type: result.app.appType,
      });
      await posthog.shutdown();
