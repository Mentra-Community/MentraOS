...
export const authCommand = new Command("auth")
  ...
  .action(async (token: string) => {
    try {
      ...
      posthog.identify(decoded.email, { email: decoded.email, key_name: decoded.name });
      posthog.capture(decoded.email, "cli_authenticated", { key_name: decoded.name, cloud: cloud.name });
...
authCommand
  ...
  .action(async () => {
    try {
      ...
      if (creds?.email) {
        posthog.capture(creds.email, "cli_logged_out");
        await posthog.shutdown();
      }
    } catch (error: any) {
      posthog.captureException(error);
