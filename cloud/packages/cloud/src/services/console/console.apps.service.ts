...
async function autoInstallAppForDeveloper(packageName: string, user: UserI): Promise<void> {
  try {
    ...
    if (user.installedApps?.some((app) => app.packageName === packageName)) {
      logger.debug({ packageName, email: user.email }, "App already installed for developer");
      return;
    }
    ...
    // Don't throw - app creation should succeed even if auto-install fails
  }
}
...
export async function createApp(
  ...
  try {
    ...
    autoInstallAppForDeveloper(packageName, user).catch((err) => {
      logger.warn({ err, packageName, email }, "Failed to auto-install app for developer");
    });
