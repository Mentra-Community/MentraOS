...
export { PhotoErrorCode, PhotoStage } from "./types/messages/glasses-to-cloud";
export type { StreamStatus, KeepAliveAck } from "./types/messages/glasses-to-cloud";
// FORK: the original alpha.3 `export { ... StreamStatus, KeepAliveAck }` was
// wrong — those are interfaces (type-only), not runtime values. Fixed by
// splitting into a `export type`. This is a regression fix that should
// propagate back to the SDK (see decisions.md).
