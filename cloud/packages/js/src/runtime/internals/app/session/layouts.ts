...
export class LayoutManager {
   ...
   */
  private createDisplayEvent(layout: Layout, view: ViewType = ViewType.MAIN, durationMs?: number): DisplayRequest {
    try {
   ...
   * ```
   */
  showDoubleTextWall(topText: string, bottomText: string, options?: { view?: ViewType; durationMs?: number }) {
   ...
   * ```
   */
  showReferenceCard(title: string, text: string, options?: { view?: ViewType; durationMs?: number }) {
   ...
   * ```
   */
  async showBitmapView(base64Bitmap: string, options?: { view?: ViewType; padding?: { left: number; top: number } }) {
   ...
   * ```
   */
  showDashboardCard(leftText: string, rightText: string, options?: { view?: ViewType; durationMs?: number }) {
    const layout: DashboardCard = {
      layoutType: LayoutType.DASHBOARD_CARD,
      leftText,
      rightText,
    };
    this.sendMessage(this.createDisplayEvent(layout, options?.view || ViewType.DASHBOARD, options?.durationMs));
  }
